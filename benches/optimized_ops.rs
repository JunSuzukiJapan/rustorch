...
fn bench_optimized_matmul(c: &mut Criterion) {
    ...
    let large_b = Tensor::from_vec((0..250000).map(|i| (i + 1) as f32).collect(), vec![500, 500]);
    ...
    let xl_b = Tensor::from_vec((0..1_000_000).map(|i| (i + 1) as f32).collect(), vec![1000, 1000]);
...
fn bench_inplace_operations(c: &mut Criterion) {
    ...
    c.bench_function("inplace_mul_scalar", |bencher| {
        bencher.iter(|| {
            let mut tensor = Tensor::from_vec((0..10000).map(|i| i as f32).collect(), vec![100, 100]);
            tensor.mul_scalar_inplace(2.0);
            black_box(tensor)
        })
    });
}
    ...
    c.bench_function("batch_matmul_32x784x128", |bencher| {
        bencher.iter(|| {
            let batch_input = Tensor::from_vec(
                (0..25088).map(|i| i as f32 * 0.01).collect(),
                vec![32, 784]
            );
...
fn bench_memory_efficiency(c: &mut Criterion) {
    c.bench_function("memory_efficient_chain", |bencher| {
        bencher.iter(|| {
            ...
            for _ in 0..5 {
                result.add_inplace(&operand);
                result.mul_scalar_inplace(0.9);
            }
