...
use rustorch::optim::{Optimizer, SGD, Adam, LAMB, AdaBound, LBFGS, LineSearchMethod};
...
fn benchmark_optimizer_convergence(c: &mut Criterion) {
    ...
    group.bench_function("SGD_convergence", |b| {
        b.iter(|| {
            let mut optimizer = SGD::new(0.01);
            let mut param = Tensor::<f32>::ones(&[problem_size, problem_size]) * 5.0;
...
fn benchmark_lbfgs_variants(c: &mut Criterion) {
    ...
    group.bench_function("L-BFGS_backtracking", |b| {
        let mut optimizer = LBFGS::with_params(0.1, 20, 20, 1e-5, 1e-9, 10, LineSearchMethod::Backtracking);
        let test_param = param.clone();
        let test_grad = grad.clone();
        b.iter(|| {
            optimizer.step(black_box(&test_param), black_box(&test_grad));
    ...
    group.bench_function("L-BFGS_strong_wolfe", |b| {
        let mut optimizer = LBFGS::with_params(0.1, 20, 20, 1e-5, 1e-9, 10, LineSearchMethod::StrongWolfe);
        let test_param = param.clone();
        let test_grad = grad.clone();
        b.iter(|| {
            optimizer.step(black_box(&test_param), black_box(&test_grad));
