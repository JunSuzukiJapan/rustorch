...
fn bench_comparison_lightweight(c: &mut Criterion) {
    ...
    group.bench_function("SVD_8x8", |b| {
        b.iter(|| black_box(&matrix).svd(false))
    });
    
    group.bench_function("QR_8x8", |b| {
        b.iter(|| black_box(&matrix).qr())
    });
    
    group.bench_function("LU_8x8", |b| {
        b.iter(|| black_box(&matrix).lu())
    });
...
fn bench_rectangular_matrices_fast(c: &mut Criterion) {
    ...
    for (rows, cols, label) in test_cases {
        ...
        // SVDとQRのみ（LUは正方行列向け）
        group.bench_with_input(
            BenchmarkId::new("SVD", label),
            &matrix,
            |b, m| b.iter(|| black_box(m.svd(false))),
        );
        
        group.bench_with_input(
            BenchmarkId::new("QR", label),
            &matrix,
            |b, m| b.iter(|| black_box(m.qr())),
        );
    }
...
fn bench_scaling_analysis(c: &mut Criterion) {
    ...
    for size in sizes {
        ...
        group.bench_with_input(
            BenchmarkId::new("SVD_scaling", size),
            &matrix,
            |b, m| b.iter(|| black_box(m.svd(false))),
        );
    }
