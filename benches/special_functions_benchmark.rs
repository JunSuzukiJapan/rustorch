...
fn benchmark_gamma_functions(c: &mut Criterion) {
    ...
    for value in test_values {
        ...
        group.bench_with_input(BenchmarkId::new("digamma_scalar", value), &value, |b, &x| {
            b.iter(|| gamma::digamma_scalar::<f64>(black_box(x)))
        });
    }
    ...
    for size in tensor_sizes {
        ...
        group.bench_with_input(
            BenchmarkId::new("gamma_tensor", size),
            &tensor,
            |b, t| b.iter(|| gamma::gamma(black_box(t))),
        );
        
        group.bench_with_input(
            BenchmarkId::new("lgamma_tensor", size),
            &tensor,
            |b, t| b.iter(|| gamma::lgamma(black_box(t))),
        );
    }
...
fn benchmark_error_functions(c: &mut Criterion) {
    ...
    for size in tensor_sizes {
        let data: Vec<f64> = (0..size).map(|i| (i as f64 - size as f64 / 2.0) * 0.1).collect();
        let tensor = Tensor::from_vec(data, vec![size]);
        
        group.bench_with_input(
            BenchmarkId::new("erf_tensor", size),
            &tensor,
            |b, t| b.iter(|| error::erf(black_box(t))),
        );
        
        group.bench_with_input(
            BenchmarkId::new("erfc_tensor", size),
            &tensor,
            |b, t| b.iter(|| error::erfc(black_box(t))),
        );
    }
