...
fn fft_benchmarks(c: &mut Criterion) {
    ...
    for size in sizes {
        ...
        let algorithm = if size.is_power_of_two() {
            "Cooley-Tukey"
        } else {
            "DFT"
        };
            ...
            |b, tensor| {
                b.iter(|| {
                    if let Ok((fft_real, fft_imag)) = tensor.fft(None, None, None) {
                        ...
                        black_box(Err::<
                            (rustorch::tensor::Tensor<f32>, rustorch::tensor::Tensor<f32>),
                            String,
                        >("FFT failed".to_string()))
                    }
                });
            },
        );
...
fn fft2_benchmarks(c: &mut Criterion) {
    ...
    for (height, width) in sizes_2d {
        ...
        let algorithm = if is_power_of_two {
            "Cooley-Tukey"
        } else {
            "Mixed"
        };
...
fn fft_normalization_benchmarks(c: &mut Criterion) {
    ...
    for (norm, name) in norms.iter().zip(norm_names.iter()) {
        group.bench_with_input(BenchmarkId::new("fft_norm", name), &tensor, |b, tensor| {
            b.iter(|| {
...
fn fft_shift_benchmarks(c: &mut Criterion) {
    ...
    for size in sizes {
        ...
        group.bench_with_input(BenchmarkId::new("fftshift", size), &tensor, |b, tensor| {
            b.iter(|| {
        ...
        group.bench_with_input(BenchmarkId::new("ifftshift", size), &tensor, |b, tensor| {
            b.iter(|| {
                ...
                b.iter(|| {
                    if let Ok((fft_real, fft_imag)) = tensor.fft(None, None, None) {
                        if let Ok((ifft_real, _ifft_imag)) =
                            tensor.ifft(&fft_real, &fft_imag, None, None, None)
                        {
                            black_box(ifft_real)
                        } else {
...
fn algorithm_comparison_benchmarks(c: &mut Criterion) {
    ...
    for size in non_power_of_2_sizes {
        ...
        group.bench_with_input(BenchmarkId::new("DFT", size), &tensor, |b, tensor| {
            b.iter(|| {
                let result = tensor.fft(None, None, None);
                black_box(result)
            });
