...
use rustorch::optimization::{OptimizationLevel, PlatformOptimizer};
use rustorch::optimization::{SimdBackend, SimdOptimizer, VectorizedOperation};
...
fn bench_simd_operations(c: &mut Criterion) {
    ...
    for size in [128, 256, 512, 1024, 2048, 4096].iter() {
        ...
        group.bench_with_input(BenchmarkId::new("auto_add", size), size, |bench, _| {
            bench.iter(|| {
...
fn bench_platform_optimizations(c: &mut Criterion) {
    ...
    for size in [1000, 10000, 100000].iter() {
        group.bench_with_input(
            ...
            size,
            |bench, &size| {
                bench.iter(|| platform_opt.allocate_aligned::<f32>(black_box(size)));
            },
        );
        ...
        group.bench_with_input(
            ...
            size,
            |bench, &size| {
                bench.iter(|| vec![0.0f32; black_box(size)]);
            },
        );
    }
...
fn bench_hardware_optimization(c: &mut Criterion) {
    ...
    for shape in [vec![1024, 1024], vec![100, 7], vec![32, 32, 3, 3]].iter() {
        group.bench_with_input(
            BenchmarkId::new("optimal_layout", format!("{:?}", shape)),
            shape,
            |bench, shape| {
                bench.iter(|| hw_optimizer.optimal_data_layout(black_box(shape)));
            },
        );
    }
...
fn bench_optimization_comparison(c: &mut Criterion) {
    ...
    group.bench_function("vectorized_dot", |bench| {
        bench.iter(|| unsafe {
            SimdOptimizer::dot_f32_avx2(black_box(&a_slice), black_box(&b_slice))
        });
    });
