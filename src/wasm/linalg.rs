...
impl WasmLinearAlgebra {
    ...
    pub fn new(max_size: usize) -> WasmLinearAlgebra {
        WasmLinearAlgebra {
            max_matrix_size: if max_size > 0 && max_size <= 1000 { max_size } else { 500 },
        }
    }
    ...
    /// 行列乗算（基本O(n³)実装）
    #[wasm_bindgen]
    pub fn matmul(&self, a: Vec<f64>, a_rows: usize, a_cols: usize, 
                  b: Vec<f64>, b_rows: usize, b_cols: usize) -> Result<Vec<f64>, JsValue> {
        if a_rows > self.max_matrix_size || a_cols > self.max_matrix_size || 
           b_rows > self.max_matrix_size || b_cols > self.max_matrix_size {
    ...
    pub fn eigenvalues(&self, matrix: Vec<f64>, n: usize) -> Result<Vec<f64>, JsValue> {
        if n > self.max_matrix_size {
            return Err(JsValue::from_str("Matrix too large for eigenvalue computation"));
        }
    ...
    /// 特異値分解（小行列用簡略版）
    #[wasm_bindgen]
    pub fn svd(&self, matrix: Vec<f64>, rows: usize, cols: usize) -> Result<js_sys::Object, JsValue> {
        ...
        let singular_values: Vec<f64> = eigenvals.iter()
        ...
        js_sys::Reflect::set(&result, &"rank".into(), &JsValue::from(sv_sorted.iter().filter(|&&x| x > 1e-10).count()))?;
        js_sys::Reflect::set(&result, &"condition_number".into(), &JsValue::from(
            if sv_sorted.last().unwrap_or(&1e-15) > &1e-15 { 
    ...
    pub fn determinant(&self, matrix: Vec<f64>, n: usize) -> Result<f64, JsValue> {
        ...
        let p_vec: Vec<usize> = (0..n).map(|i| p_array.get(i as u32).as_f64().unwrap() as usize).collect();
        for i in 0..n {
            if p_vec[i] != i {
                swap_count += 1;
            }
    ...
    pub fn inverse(&self, matrix: Vec<f64>, n: usize) -> Result<Vec<f64>, JsValue> {
        ...
        let l_vec: Vec<f64> = (0..n*n).map(|i| l_array.get(i as u32).as_f64().unwrap()).collect();
        let u_vec: Vec<f64> = (0..n*n).map(|i| u_array.get(i as u32).as_f64().unwrap()).collect();
        let p_vec: Vec<usize> = (0..n).map(|i| p_array.get(i as u32).as_f64().unwrap() as usize).collect();
    ...
    /// 行列の条件数を計算
    #[wasm_bindgen]
    pub fn condition_number(&self, matrix: Vec<f64>, rows: usize, cols: usize) -> Result<f64, JsValue> {
    ...
    /// SVDによる擬似逆行列計算
    #[wasm_bindgen]
    pub fn pseudoinverse(&self, matrix: Vec<f64>, rows: usize, cols: usize) -> Result<Vec<f64>, JsValue> {
        if rows > self.max_matrix_size || cols > self.max_matrix_size {
            return Err(JsValue::from_str("Matrix too large"));
        }
    ...
    /// べき乗法による最大固有値計算
    #[wasm_bindgen]
    pub fn largest_eigenvalue(&self, matrix: Vec<f64>, n: usize, max_iter: usize) -> Result<f64, JsValue> {
        if n > self.max_matrix_size {
            return Err(JsValue::from_str("Matrix too large"));
        }
        ...
        for _ in 0..max_iter {
            ...
            // Check convergence
            let diff: f64 = new_v.iter().zip(v.iter()).map(|(a, b)| (a - b) * (a - b)).sum::<f64>().sqrt();
            if diff < 1e-10 {
                break;
            }
    ...
    /// 行列ベクトル積
    #[wasm_bindgen]
    pub fn matvec(&self, matrix: Vec<f64>, vector: Vec<f64>, rows: usize, cols: usize) -> Result<Vec<f64>, JsValue> {
...
impl WasmLinAlgUtils {
    ...
    pub fn random_positive_definite(n: usize, scale: f64) -> Vec<f64> {
        ...
        WasmLinearAlgebra::new(1000).matmul(at, n, n, a, n, n).unwrap()
    }
