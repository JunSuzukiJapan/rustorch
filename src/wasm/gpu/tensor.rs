...
impl WebGPUTensorEngine {
    ...
    pub async fn add_tensors(&mut self, a_label: &str, b_label: &str) -> Result<String, JsValue> {
        ...
        // Execute addition
        self.context.tensor_add(a_label, b_label, &output_label).await?;
    ...
    pub async fn mul_tensors(&mut self, a_label: &str, b_label: &str) -> Result<String, JsValue> {
        ...
        self.context.tensor_mul(a_label, b_label, &output_label).await?;
        
        Ok(output_label)
    }

    #[wasm_bindgen]
    pub async fn matmul_tensors(&mut self, a_label: &str, b_label: &str, m: u32, n: u32, k: u32) -> Result<String, JsValue> {
        ...
        self.context.tensor_matmul(a_label, b_label, &output_label, m, n, k).await?;
    ...
    pub async fn sigmoid_tensor(&mut self, input_label: &str) -> Result<String, JsValue> {
        ...
        // Execute Sigmoid
        self.context.tensor_sigmoid(input_label, &output_label).await?;
...
impl WebGPUTensorEngine {
    ...
    pub async fn webgpu_to_tensor(&self, buffer_label: &str, shape: Vec<usize>) -> Result<Tensor<f32>, JsValue> {
        let data = self.download_tensor_f32(buffer_label).await?;
        
        match Tensor::try_from_vec(data, shape) {
            Ok(tensor) => Ok(tensor),
            Err(e) => Err(JsValue::from_str(&format!("Failed to create tensor: {}", e)))
        }
    }
}
