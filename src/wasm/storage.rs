...
impl WasmModelStorage {
    ...
    pub fn new(use_indexeddb: bool, chunk_size: usize) -> WasmModelStorage {
        WasmModelStorage {
            use_indexeddb,
            chunk_size: if chunk_size > 0 { chunk_size } else { 1_000_000 },
        }
    }
    ...
    #[wasm_bindgen]
    pub async fn save_large_model(&self, model_name: &str, model_data: Vec<u8>) -> Result<(), JsValue> {
        let chunk_count = (model_data.len() + self.chunk_size - 1) / self.chunk_size;
        
        // Save metadata
        let metadata = js_sys::Object::new();
        js_sys::Reflect::set(&metadata, &"total_size".into(), &JsValue::from(model_data.len()))?;
        js_sys::Reflect::set(&metadata, &"chunk_count".into(), &JsValue::from(chunk_count))?;
        js_sys::Reflect::set(&metadata, &"chunk_size".into(), &JsValue::from(self.chunk_size))?;
        ...
        if self.use_indexeddb {
            ...
            if let Ok(storage) = js_sys::Reflect::get(&navigator, &"storage".into()) {
                if let Ok(estimate_func) = js_sys::Reflect::get(&storage, &"estimate".into()) {
                    ...
                    let estimate_result = JsFuture::from(js_sys::Promise::from(estimate_promise)).await?;
            ...
            let storage = window()?.local_storage()?.ok_or("LocalStorage not available")?;
    ...
    async fn save_to_indexeddb(&self, key: &str, data: &[u8]) -> Result<(), JsValue> {
        ...
        let db_promise = js_sys::Promise::new(&mut |resolve, reject| {
            ...
            let onerror = Closure::wrap(Box::new(move |_: web_sys::Event| {
                reject.call1(&JsValue::UNDEFINED, &JsValue::from_str("Database error")).unwrap();
            }) as Box<dyn FnMut(_)>);
        ...
        // Store data
        let transaction = db.transaction_with_str_and_mode("models", web_sys::IdbTransactionMode::Readwrite)?;
    ...
    async fn load_from_indexeddb(&self, key: &str) -> Result<Vec<u8>, JsValue> {
        ...
        let get_promise = js_sys::Promise::new(&mut |resolve, reject| {
            ...
            let onerror = Closure::wrap(Box::new(move |_: web_sys::Event| {
                reject.call1(&JsValue::UNDEFINED, &JsValue::from_str("Get error")).unwrap();
            }) as Box<dyn FnMut(_)>);
    ...
    async fn list_from_indexeddb(&self) -> Result<Vec<String>, JsValue> {
        ...
        let keys_promise = js_sys::Promise::new(&mut |resolve, reject| {
            ...
            let onerror = Closure::wrap(Box::new(move |_: web_sys::Event| {
                reject.call1(&JsValue::UNDEFINED, &JsValue::from_str("Keys error")).unwrap();
            }) as Box<dyn FnMut(_)>);
...
impl WasmModelCompression {
    ...
    pub fn decompress_weights(compressed: Vec<u8>) -> Vec<f32> {
        ...
        while i + 4 < compressed.len() {
            ...
            let value_bytes = [compressed[i+1], compressed[i+2], compressed[i+3], compressed[i+4]];
...
fn base64_decode(encoded: &str) -> Result<Vec<u8>, JsValue> {
    ...
    let chars: Vec<u8> = encoded.bytes().filter(|&c| c != b'=' && c != b'\n' && c != b'\r').collect();
