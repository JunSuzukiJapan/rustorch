    ...
    T: rand::distributions::uniform::SampleUniform + num_traits::FromPrimitive + std::fmt::Display,
{
    ...
    pub fn new(loc: Tensor<T>, scale: Tensor<T>, validate_args: bool) -> Result<Self, DistributionError> {
        if validate_args {
    ...
    fn log_normalizing_constant(&self) -> Result<Tensor<T>, DistributionError> {
        ...
        let result_data: Vec<T> = log_scale.iter()
            .map(|&ls| ls + half_log_2pi)
            .collect();
    ...
    fn erf_approx(x: T) -> T {
        ...
        let sign = if x >= T::zero() { T::one() } else { T::from(-1.0).unwrap() };
        let x_abs = x.abs();
    ...
    T: rand::distributions::uniform::SampleUniform + num_traits::FromPrimitive + std::fmt::Display,
{
    fn sample(&self, shape: Option<&[usize]>) -> Result<Tensor<T>, DistributionError> {
        ...
        let result_data: Vec<T> = standard_data.iter()
            .zip(loc_data.iter().cycle())
            .zip(scale_data.iter().cycle())
            .map(|((&z, &l), &s)| l + s * z)
            .collect();
    ...
    fn log_prob(&self, value: &Tensor<T>) -> Result<Tensor<T>, DistributionError> {
        ...
        let result_data: Vec<T> = std_data.iter()
    ...
    fn entropy(&self) -> Result<Tensor<T>, DistributionError> {
        ...
        let result_data: Vec<T> = scale_data.iter()
            .map(|&s| s.ln() + constant)
            .collect();
...
mod tests {
    ...
    fn test_normal_creation() {
        ...
        assert_eq!(normal.base.event_shape, vec![] as Vec<usize>);
    }
