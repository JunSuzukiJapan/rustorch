...
use crate::distributions::{DistributionTrait, DistributionError, Distribution, DistributionUtils};
    ...
    T: rand::distributions::uniform::SampleUniform + num_traits::FromPrimitive + std::fmt::Display,
{
    ...
    pub fn new(concentration1: Tensor<T>, concentration0: Tensor<T>, validate_args: bool) -> Result<Self, DistributionError> {
        if validate_args {
        ...
        let batch_shape = Distribution::broadcast_shapes(concentration1.shape(), concentration0.shape())?;
    ...
    fn log_beta(&self) -> Result<Tensor<T>, DistributionError> {
        ...
        let result_data: Vec<T> = alpha_data.iter()
            .zip(beta_data.iter().cycle())
            .map(|(&a, &b)| {
                // Use Stirling's approximation for log-gamma
                Self::log_gamma_stirling(a) + Self::log_gamma_stirling(b) - Self::log_gamma_stirling(a + b)
            })
            .collect();
            
        Ok(Tensor::from_vec(result_data, self.concentration1.shape().to_vec()))
    }
    ...
    T: rand::distributions::uniform::SampleUniform + num_traits::FromPrimitive + std::fmt::Display,
{
    ...
    fn log_prob(&self, value: &Tensor<T>) -> Result<Tensor<T>, DistributionError> {
        ...
        let result_data: Vec<T> = value_data.iter()
    ...
    fn mean(&self) -> Result<Tensor<T>, DistributionError> {
        ...
        let mean_data: Vec<T> = alpha_data.iter()
            .zip(beta_data.iter().cycle())
            .map(|(&a, &b)| a / (a + b))
            .collect();
            
        Ok(Tensor::from_vec(mean_data, self.concentration1.shape().to_vec()))
    }
    ...
    fn variance(&self) -> Result<Tensor<T>, DistributionError> {
        ...
        let var_data: Vec<T> = alpha_data.iter()
            .zip(beta_data.iter().cycle())
            .map(|(&a, &b)| {
                let sum = a + b;
        ...
        Ok(Tensor::from_vec(var_data, self.concentration1.shape().to_vec()))
    }
    ...
    fn entropy(&self) -> Result<Tensor<T>, DistributionError> {
        ...
        let result_data: Vec<T> = alpha_data.iter()
            ...
            .map(|((&a, &b), &lb)| {
                ...
                lb - (a - T::one()) * Self::digamma(a) 
                   - (b - T::one()) * Self::digamma(b)
    ...
    T: rand::distributions::uniform::SampleUniform + num_traits::FromPrimitive + std::fmt::Display,
{
    ...
    fn incomplete_beta_approx(x: T, a: T, b: T) -> T {
        ...
        let beta_approx = x.powf(a) * (T::one() - x).powf(b) / (a * Self::beta_function_approx(a, b));
        beta_approx * cf
    }
    ...
    fn beta_function_approx(a: T, b: T) -> T {
        (Self::log_gamma_stirling(a) + Self::log_gamma_stirling(b) - Self::log_gamma_stirling(a + b)).exp()
    }
}
...
mod tests {
    ...
    fn test_beta_mean_variance() {
        ...
        assert_abs_diff_eq!(var.data.as_slice().unwrap()[0], expected_var, epsilon = 1e-6);
    }
