    ...
    T: rand::distributions::uniform::SampleUniform + num_traits::FromPrimitive + std::fmt::Display,
{
    ...
    pub fn new(low: Tensor<T>, high: Tensor<T>, validate_args: bool) -> Result<Self, DistributionError> {
        if validate_args {
            // Validate low < high
            ...
            for (i, (&l, &h)) in low_data.iter().zip(high_data.iter().cycle()).enumerate() {
                if l >= h {
                    return Err(DistributionError::InvalidParameter(
                        format!("low must be less than high, got low[{}] = {}, high[{}] = {}", 
                                i, l, i, h)
                    ));
                }
            }
    ...
    fn range(&self) -> Tensor<T> {
        ...
        let range_data: Vec<T> = low_data.iter()
    ...
    T: rand::distributions::uniform::SampleUniform + num_traits::FromPrimitive + std::fmt::Display,
{
    ...
    fn mean(&self) -> Result<Tensor<T>, DistributionError> {
        ...
        let mean_data: Vec<T> = low_data.iter()
            .zip(high_data.iter().cycle())
            .map(|(&l, &h)| half * (l + h))
            .collect();
    ...
    fn variance(&self) -> Result<Tensor<T>, DistributionError> {
        ...
        let var_data: Vec<T> = range_data.iter()
            .map(|&r| (r * r) / twelve)
            .collect();
    ...
    fn entropy(&self) -> Result<Tensor<T>, DistributionError> {
        ...
        let entropy_data: Vec<T> = range_data.iter()
            .map(|&r| r.ln())
            .collect();
...
mod tests {
    ...
    fn test_uniform_sampling() {
        ...
        assert_abs_diff_eq!(sample_mean, 3.5, epsilon = 0.1); // Should be (2+5)/2 = 3.5
    }
