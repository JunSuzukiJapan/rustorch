    ...
    T: rand::distributions::uniform::SampleUniform + num_traits::FromPrimitive + std::fmt::Display,
{
    ...
    pub fn from_scale(scale: Tensor<T>, validate_args: bool) -> Result<Self, DistributionError> {
        ...
        let rate_data: Vec<T> = scale_data.iter()
            .map(|&s| T::one() / s)
            .collect();
    ...
    pub fn scale(&self) -> Tensor<T> {
        ...
        let scale_data: Vec<T> = rate_data.iter()
            .map(|&r| T::one() / r)
            .collect();
    ...
    T: rand::distributions::uniform::SampleUniform + num_traits::FromPrimitive + std::fmt::Display,
{
    fn sample(&self, shape: Option<&[usize]>) -> Result<Tensor<T>, DistributionError> {
        ...
        let result_data: Vec<T> = uniform_data.iter()
            .zip(rate_data.iter().cycle())
            .map(|(&u, &lambda)| {
                // Handle edge case where u might be exactly 0
                let safe_u = if u <= T::zero() { T::from(1e-10).unwrap() } else { u };
                -safe_u.ln() / lambda
            })
            .collect();
    ...
    fn variance(&self) -> Result<Tensor<T>, DistributionError> {
        // Variance = 1/λ²
        let rate_data = self.rate.data.as_slice().unwrap();
        let var_data: Vec<T> = rate_data.iter()
    ...
    fn entropy(&self) -> Result<Tensor<T>, DistributionError> {
        // Entropy = 1 - log(λ)
        let rate_data = self.rate.data.as_slice().unwrap();
        let entropy_data: Vec<T> = rate_data.iter()
...
mod tests {
    ...
    fn test_exponential_creation() {
        ...
        assert_eq!(exp_dist.base.event_shape, vec![] as Vec<usize>);
    }
    ...
    fn test_exponential_log_prob() {
        ...
        assert_abs_diff_eq!(log_prob_data[3], 2.0f32.ln() - 4.0, epsilon = 1e-6); // x = 2
    }
    ...
    fn test_exponential_properties() {
        ...
        assert_abs_diff_eq!(var.data.as_slice().unwrap()[0], 1.0/(rate*rate), epsilon = 1e-6);
        assert_abs_diff_eq!(entropy.data.as_slice().unwrap()[0], 1.0 - rate.ln(), epsilon = 1e-6);
    }
