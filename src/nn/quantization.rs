...
use crate::nn::Module;
use std::fmt::Debug;
use num_traits::{Float, FromPrimitive, ToPrimitive, Zero, One, Signed};
...
where
    T: Float + FromPrimitive + ToPrimitive + Debug + Default + Zero + One + Send + Sync + Copy + ScalarOperand + Sum + Signed,
{
    /// Create a new quantized tensor
    /// 新しい量子化テンソルを作成
    pub fn new(data: Vec<i8>, shape: Vec<usize>, params: QuantizationParams<T>) -> Self {
        QuantizedTensor { data, shape, params }
    }
    ...
    T: Float + FromPrimitive + ToPrimitive + Debug + Default + Zero + One + Send + Sync + Copy + ScalarOperand + Sum + Signed,
{
    ...
    /// テンソル用の量子化パラメータを計算
    fn compute_quantization_params(&self, tensor: &Tensor<T>, qtype: QuantizationType) -> QuantizationParams<T> {
        let tensor_array = tensor.as_array();
        let tensor_slice = tensor_array.as_slice().unwrap_or_else(|| {
            panic!("Cannot get slice from tensor")
        });
        
        let (min_val, max_val) = match self.calibration_mode {
            CalibrationMode::MinMax => {
                let min = tensor_slice.iter().fold(T::infinity(), |a, &b| if a < b { a } else { b });
                let max = tensor_slice.iter().fold(T::neg_infinity(), |a, &b| if a > b { a } else { b });
                (min, max)
            },
...
where
    T: Float + FromPrimitive + ToPrimitive + Debug + Default + Zero + One + Send + Sync + Copy + ScalarOperand + Sum + Signed + 'static,
    M: Module<T> + 'static,
{
    ...
    fn apply_fake_quantization(&self, input: &Variable<T>, layer_name: &str) -> Variable<T> {
        ...
        if let Some(params) = self.qparams.get(layer_name) {
            ...
            let quantized_data: Vec<T> = input_data.as_array().iter().map(|&val| {
                let scaled = val / params.scale;
                let quantized = scaled.round().to_i32().unwrap_or(0) + params.zero_point;
                let clamped = quantized.max(params.qmin).min(params.qmax);
            ...
            let fake_quantized_tensor = Tensor::from_vec(quantized_data, input_data.shape().to_vec());
