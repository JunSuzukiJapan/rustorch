    ...
    T: Float + Debug + Default + From<f32> + 'static + Send + Sync + Copy,
{
    ...
    pub fn new(
        ...
        assert!(kernel_size.0 > 0 && kernel_size.1 > 0 && kernel_size.2 > 0, "kernel_size must be positive");
        assert!(stride.0 > 0 && stride.1 > 0 && stride.2 > 0, "stride must be positive");
        assert!(dilation.0 > 0 && dilation.1 > 0 && dilation.2 > 0, "dilation must be positive");
    ...
    pub fn num_parameters(&self) -> usize {
        let weight_params = self.out_channels * (self.in_channels / self.groups) * 
...
mod tests {
    ...
    fn test_output_size_calculation() {
        let layer: Conv3d<f32> = Conv3d::new(
            16, 32, (3, 3, 3), Some((1, 1, 1)), Some((1, 1, 1)), None, None, Some(true)
        );
    ...
    fn test_num_parameters() {
        let layer: Conv3d<f32> = Conv3d::new(
            32, 64, (3, 3, 3), None, None, None, None, Some(true)
        );
    ...
    fn test_receptive_field() {
        let layer: Conv3d<f32> = Conv3d::new(
            16, 32, (3, 5, 5), None, None, Some((2, 1, 1)), None, None
        );
    ...
    fn test_parameters() {
        let layer: Conv3d<f32> = Conv3d::new(
            8, 16, (2, 2, 2), None, None, None, None, Some(true)
        );
    ...
    fn test_no_bias() {
        let layer: Conv3d<f32> = Conv3d::new(
            8, 16, (2, 2, 2), None, None, None, None, Some(false)
        );
    ...
    fn test_grouped_convolution() {
        let layer: Conv3d<f32> = Conv3d::new(
            32, 64, (2, 2, 2), None, None, None, Some(4), Some(true)
        );
    ...
    fn test_output_size_with_stride() {
        let layer: Conv3d<f32> = Conv3d::new(
            16, 32, (3, 3, 3), Some((2, 2, 2)), Some((0, 0, 0)), None, None, None
        );
