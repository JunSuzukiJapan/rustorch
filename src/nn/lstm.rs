    ...
    T: Float + Debug + Default + From<f32> + 'static + Send + Sync + Copy,
{
    ...
    pub fn new(input_size: usize, hidden_size: usize, bias: Option<bool>) -> Self {
        ...
        // Each gate has its own set of weights, so we need 4x the parameters
        let std_ih = (6.0 / (input_size + hidden_size * 4) as f32).sqrt();
        let std_hh = (6.0 / (hidden_size + hidden_size * 4) as f32).sqrt();
        
        // Weight matrices: [4 * hidden_size, input_size/hidden_size]
        // For input-to-hidden: [4 * hidden_size, input_size]
        // For hidden-to-hidden: [4 * hidden_size, hidden_size]
        let weight_ih = Self::init_weight([4 * hidden_size, input_size], std_ih);
        let weight_hh = Self::init_weight([4 * hidden_size, hidden_size], std_hh);
        
        let bias_ih = if use_bias {
            Some(Self::init_bias(4 * hidden_size))
        } else {
            None
        };
    ...
    /// * `hidden` - Tuple of (h_{t-1}, c_{t-1}) each of shape [batch_size, hidden_size]
    /// 
    /// # Returns
    /// Tuple of (h_t, c_t) each of shape [batch_size, hidden_size]
    /// 
    /// # 引数
    /// * `input` - 形状[batch_size, input_size]の入力テンソル
    /// * `hidden` - (h_{t-1}, c_{t-1})のタプル、それぞれ形状[batch_size, hidden_size]
    /// 
    /// # 戻り値
    /// (h_t, c_t)のタプル、それぞれ形状[batch_size, hidden_size]
    pub fn forward(
        &self,
        input: &Variable<T>,
        hidden: (&Variable<T>, &Variable<T>),
    ) -> (Variable<T>, Variable<T>) {
        let (h_prev, c_prev) = hidden;
        ...
        let hh_out = self.linear_transform(h_prev, &self.weight_hh, &self.bias_hh);
        
        // Add transformations
        let combined = self.add_tensors(&ih_out, &hh_out);
        
        // Split into 4 gates: input, forget, cell, output
        let gates = self.split_gates(&combined);
        
        // Apply activations
        let input_gate = self.sigmoid_activation(&gates[0]);
        let forget_gate = self.sigmoid_activation(&gates[1]);
        let cell_gate = self.tanh_activation(&gates[2]);
        let output_gate = self.sigmoid_activation(&gates[3]);
        
        // Compute new cell state: c_t = f_t ⊙ c_{t-1} + i_t ⊙ g_t
        let forget_part = self.element_multiply(&forget_gate, c_prev);
        let input_part = self.element_multiply(&input_gate, &cell_gate);
        let new_cell = self.add_tensors(&forget_part, &input_part);
        
        // Compute new hidden state: h_t = o_t ⊙ tanh(c_t)
        let cell_tanh = self.tanh_activation(&new_cell);
        let new_hidden = self.element_multiply(&output_gate, &cell_tanh);
    ...
    /// Split concatenated gates into 4 separate tensors
    /// 連結されたゲートを4つの別々のテンソルに分割
    fn split_gates(&self, combined: &Variable<T>) -> Vec<Variable<T>> {
        ...
        let gate_size = total_features / 4;
        
        assert_eq!(total_features % 4, 0, "Total features must be divisible by 4");
        ...
        for gate_idx in 0..4 {
            ...
            for i in 0..batch_size {
                for j in 0..gate_size {
                    let feature_idx = gate_idx * gate_size + j;
                    gate_data.push(combined_array[[i, feature_idx]]);
                }
            }
    ...
    /// ゼロで隠れ状態とセル状態を初期化
    pub fn init_hidden(&self, batch_size: usize) -> (Variable<T>, Variable<T>) {
        let hidden_data = vec![<T as From<f32>>::from(0.0f32); batch_size * self.hidden_size];
        let cell_data = vec![<T as From<f32>>::from(0.0f32); batch_size * self.hidden_size];
        
        let hidden = Variable::new(
            Tensor::from_vec(hidden_data, vec![batch_size, self.hidden_size]),
            false,
        );
        let cell = Variable::new(
            Tensor::from_vec(cell_data, vec![batch_size, self.hidden_size]),
            false,
        );
    ...
    T: Float + Debug + Default + From<f32> + 'static + Send + Sync + Copy,
{
    fn forward(&self, input: &Variable<T>) -> Variable<T> {
        // For single-step forward, we need hidden and cell states
        // This is a simplified implementation - normally you'd pass states
        let input_binding = input.data();
        let input_data = input_binding.read().unwrap();
        let batch_size = input_data.shape()[0];
        let (hidden, cell) = self.init_hidden(batch_size);
        let (new_hidden, _) = self.forward(input, (&hidden, &cell));
        new_hidden
    }
...
/// 多層LSTMの実装
/// 
/// A multi-layer LSTM that processes sequences of inputs
/// 入力シーケンスを処理する多層LSTM
#[derive(Debug)]
    ...
    T: Float + Debug + Default + From<f32> + 'static + Send + Sync + Copy,
{
    ...
    /// * `bidirectional` - trueの場合、双方向LSTMを使用
    pub fn new(
        input_size: usize,
        hidden_size: usize,
        ...
        for i in 0..num_layers {
            ...
            // Add backward layer for bidirectional LSTM
            if bidirectional {
                layers.push(LSTMCell::new(layer_input_size, hidden_size, bias));
            }
        }
    ...
    /// * `hidden` - Initial (hidden, cell) states (optional)
    /// 
    /// # Returns
    /// (output, (final_hidden, final_cell)) where:
    /// - output: [seq_len, batch_size, hidden_size * num_directions]
    /// - final_hidden: [num_layers * num_directions, batch_size, hidden_size]
    /// - final_cell: [num_layers * num_directions, batch_size, hidden_size]
    /// 
    /// # 引数
    /// * `input` - 形状[seq_len, batch_size, input_size]の入力シーケンステンソル
    /// * `hidden` - 初期(隠れ状態, セル状態)（オプション）
    /// 
    /// # 戻り値
    ...
    pub fn forward_with_hidden(
        ...
        let (mut current_hidden, mut current_cell) = if let Some((h, c)) = hidden {
            (h.clone(), c.clone())
        } else {
        ...
        for layer_idx in 0..self.num_layers {
            ...
            let (mut step_hidden, mut step_cell) = self.extract_layer_states(&current_hidden, &current_cell, layer_idx);
            
            // Forward direction
            for t in 0..seq_len {
                let step_input = self.extract_timestep(&current_layer_input, t);
                let (new_hidden, new_cell) = cell.forward(&step_input, (&step_hidden, &step_cell));
                step_hidden = new_hidden;
                step_cell = new_cell;
                layer_output.push(step_hidden.clone());
            }
            
            let mut layer_final_hiddens = vec![step_hidden];
            let mut layer_final_cells = vec![step_cell];
            
            // Backward direction (if bidirectional)
            if self.bidirectional {
                let backward_cell = &self.layers[cell_idx + 1];
                let mut backward_output = Vec::new();
                let (mut backward_hidden, mut backward_cell_state) = 
                    self.extract_layer_states(&current_hidden, &current_cell, layer_idx);
                
                for t in (0..seq_len).rev() {
                    let step_input = self.extract_timestep(&current_layer_input, t);
                    let (new_hidden, new_cell) = backward_cell.forward(&step_input, (&backward_hidden, &backward_cell_state));
                    backward_hidden = new_hidden;
                    backward_cell_state = new_cell;
                    backward_output.push(backward_hidden.clone());
                }
                
                backward_output.reverse();
                layer_final_hiddens.push(backward_hidden);
                layer_final_cells.push(backward_cell_state);
            ...
            // Update current_hidden and current_cell for this layer only
            current_hidden = self.update_layer_hidden(&current_hidden, layer_idx, &layer_final_hiddens);
            current_cell = self.update_layer_hidden(&current_cell, layer_idx, &layer_final_cells);
        }
        
        let output = self.stack_sequence_outputs(&layer_outputs);
        (output, (current_hidden, current_cell))
    }
    
    /// Initialize hidden and cell states
    /// 隠れ状態とセル状態を初期化
    fn init_hidden(&self, batch_size: usize) -> (Variable<T>, Variable<T>) {
        ...
        let hidden_data = vec![<T as From<f32>>::from(0.0f32); total_hidden_size];
        let cell_data = vec![<T as From<f32>>::from(0.0f32); total_hidden_size];
        
        let hidden = Variable::new(
            Tensor::from_vec(hidden_data, vec![self.num_layers * num_directions, batch_size, hidden_size]),
            false,
        );
        let cell = Variable::new(
            Tensor::from_vec(cell_data, vec![self.num_layers * num_directions, batch_size, hidden_size]),
            false,
        );
    ...
    /// Extract hidden and cell states for a specific layer
    /// 特定の層の隠れ状態とセル状態を抽出
    fn extract_layer_states(
        &self,
        hidden: &Variable<T>,
        cell: &Variable<T>,
        layer_idx: usize,
        ...
        let hidden_binding = hidden.data();
        let hidden_data = hidden_binding.read().unwrap();
        let cell_binding = cell.data();
        let cell_data = cell_binding.read().unwrap();
        ...
        let layer_hidden = Variable::new(
            Tensor::from_vec(layer_hidden_data, vec![batch_size, hidden_size]),
            hidden.requires_grad(),
        );
        let layer_cell = Variable::new(
            Tensor::from_vec(layer_cell_data, vec![batch_size, hidden_size]),
            cell.requires_grad(),
        );
        
        (layer_hidden, layer_cell)
    }
    ...
    fn stack_hidden_states(&self, hiddens: &[Variable<T>]) -> Variable<T> {
        ...
        let hidden_binding = first_hidden.data();
        let hidden_data = hidden_binding.read().unwrap();
        let hidden_shape = hidden_data.shape();
...
mod tests {
    ...
    fn test_lstm_cell_creation() {
        let cell = LSTMCell::<f32>::new(10, 20, Some(true));
    ...
    fn test_lstm_cell_forward() {
        let cell = LSTMCell::<f32>::new(3, 5, Some(true));
        ...
        let (hidden, cell_state) = cell.init_hidden(2);
        let (new_hidden, new_cell) = cell.forward(&input, (&hidden, &cell_state));
        
        let hidden_binding = new_hidden.data();
        let hidden_data = hidden_binding.read().unwrap();
        let cell_binding = new_cell.data();
        let cell_data = cell_binding.read().unwrap();
        
        assert_eq!(hidden_data.shape(), &[2, 5]);
        assert_eq!(cell_data.shape(), &[2, 5]);
        
        // Output should be different from zero (since we have non-zero input)
        let hidden_array = hidden_data.as_array();
        assert!(hidden_array.iter().any(|&x| x != 0.0));
    }
    
    #[test]
    fn test_lstm_cell_no_bias() {
        let cell = LSTMCell::<f32>::new(5, 10, Some(false));
        let params = cell.parameters();
    ...
    fn test_lstm_multi_layer() {
        ...
        let (output, (hidden, cell)) = lstm.forward_with_hidden(&input, None);
        ...
        assert_eq!(cell_data.shape(), &[2, 3, 20]); // [num_layers, batch, hidden_size]
    }
    ...
    fn test_lstm_bidirectional() {
        ...
        assert_eq!(cell_data.shape(), &[2, 2, 10]); // [num_layers * 2, batch, hidden_size]
    }
    
    #[test]
    fn test_init_hidden() {
        let cell = LSTMCell::<f32>::new(5, 10, Some(true));
        let (hidden, cell_state) = cell.init_hidden(3);
        
        let hidden_binding = hidden.data();
        let hidden_data = hidden_binding.read().unwrap();
        let cell_binding = cell_state.data();
        let cell_data = cell_binding.read().unwrap();
        
        assert_eq!(hidden_data.shape(), &[3, 10]);
        assert_eq!(cell_data.shape(), &[3, 10]);
