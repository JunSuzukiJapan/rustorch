...
impl SafeOps {
        ...
        T: Float + Send + Sync + 'static + Debug + Clone + PartialOrd + ndarray::ScalarOperand + num_traits::FromPrimitive,
    {
        ...
        let min = data.iter().fold(first, |acc, &x| if x < acc { x } else { acc });
        let max = data.iter().fold(first, |acc, &x| if x > acc { x } else { acc });
...
mod tests {
    ...
    fn test_finite_validation() {
        ...
        let nan_var = SafeOps::create_variable(vec![1.0, f32::NAN, 3.0, 4.0], vec![2, 2], false).unwrap();
        assert!(SafeOps::validate_finite(&nan_var).is_err());
        
        let inf_var = SafeOps::create_variable(vec![1.0, f32::INFINITY, 3.0, 4.0], vec![2, 2], false).unwrap();
