...
pub trait NormalizationFunction<T> {
    ...
    /// Batch normalization
    /// バッチ正規化
    fn batch_norm(&self, input: Self::Input, gamma: &[T], beta: &[T], 
                  running_mean: &mut [T], running_var: &mut [T], 
                  momentum: T, epsilon: T, training: bool) -> Result<Self::Output, Self::Error>;
    
    /// Layer normalization
    /// レイヤー正規化
    fn layer_norm(&self, input: Self::Input, gamma: &[T], beta: &[T], 
                  epsilon: T) -> Result<Self::Output, Self::Error>;
    
    /// Group normalization
    /// グループ正規化
    fn group_norm(&self, input: Self::Input, gamma: &[T], beta: &[T], 
                  num_groups: usize, epsilon: T) -> Result<Self::Output, Self::Error>;
}
...
pub mod shared_normalizations {
    ...
    pub fn compute_mean<T: Float>(data: &[T]) -> T {
        ...
        T::from(data.len()).unwrap_or(T::one())
    }
