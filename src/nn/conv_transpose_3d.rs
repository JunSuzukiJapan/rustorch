    ...
    T: Float + Debug + Default + From<f32> + 'static + Send + Sync + Copy + ndarray::ScalarOperand + num_traits::FromPrimitive,
{
    ...
    pub fn new(
        ...
        assert!(output_padding.2 < stride.2, "output_padding.2 must be less than stride.2");
        ...
        let fan_in = (out_channels / groups) * kernel_size.0 * kernel_size.1 * kernel_size.2;
    ...
    pub fn forward(&self, input: &Variable<T>) -> Variable<T> {
        ...
        let (output_d, output_h, output_w) = self.calculate_output_size((input_d, input_h, input_w));
    ...
    pub fn calculate_output_size(&self, input_size: (usize, usize, usize)) -> (usize, usize, usize) {
        ...
        for b in 0..batch_size {
            ...
            for in_ch in 0..self.in_channels {
                ...
                for out_ch in out_ch_start..out_ch_end {
                    ...
                    let output_ch_offset = output_batch_offset + out_ch * output_d * output_h * output_w;
                    let weight_offset = in_ch * (self.out_channels / self.groups) * kernel_d * kernel_h * kernel_w
                        + out_ch_in_group * kernel_d * kernel_h * kernel_w;
                    
                    for i_d in 0..input_d {
                        for i_h in 0..input_h {
                            for i_w in 0..input_w {
                                let input_idx = input_ch_offset + i_d * input_h * input_w + i_h * input_w + i_w;
                                ...
                                for k_d in 0..kernel_d {
                                    for k_h in 0..kernel_h {
                                        for k_w in 0..kernel_w {
                                            ...
                                            if out_d >= pad_d && out_h >= pad_h && out_w >= pad_w
                                                ...
                                                && out_w < output_w + pad_w - out_pad_w
                                            {
                                                ...
                                                if adj_out_d < output_d && adj_out_h < output_h && adj_out_w < output_w {
                                                    let output_idx = output_ch_offset + adj_out_d * output_h * output_w + adj_out_h * output_w + adj_out_w;
                                                    let weight_idx = weight_offset + k_d * kernel_h * kernel_w + k_h * kernel_w + k_w;
    ...
    fn add_bias(&self, output: &mut [T], output_shape: &[usize], bias: &[T]) {
        ...
        let output_h = output_shape[3];
        let output_w = output_shape[4];
        let spatial_size = output_d * output_h * output_w;
        
        for b in 0..batch_size {
            for ch in 0..self.out_channels {
                let ch_offset = b * self.out_channels * spatial_size + ch * spatial_size;
...
mod tests {
    ...
    fn test_output_size_calculation_3d() {
        let layer: ConvTranspose3d<f32> = ConvTranspose3d::new(
            4, 8, (3, 3, 3), Some((2, 2, 2)), Some((1, 1, 1)), None, None, None, Some(true)
        );
