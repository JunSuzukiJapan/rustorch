...
impl MemoryTracker {
    ...
    pub fn track_allocation(&mut self, component: &str, size_bytes: usize) -> RusTorchResult<usize> {
        if !self.enabled {
            return Ok(0);
        }
        ...
        while new_usage > peak {
            match self.peak_usage.compare_exchange_weak(peak, new_usage, Ordering::SeqCst, Ordering::Relaxed) {
                Ok(_) => break,
                Err(x) => peak = x,
            }
        }
        ...
        let allocation_info = AllocationInfo::new(
            component.to_string(),
            size_bytes,
            allocation_id
        );
        ...
        // Store allocation
        self.allocations.insert(allocation_id, allocation_info.clone());
        ...
        let session_duration = self.session_start.elapsed().unwrap_or_default().as_secs_f64();
        ...
        if stats.total_allocations > 0 {
            stats.average_allocation_size = stats.current_usage_bytes as f64 / stats.current_allocations as f64;
        }
        ...
        if size_bytes > self.large_allocation_threshold_mb * 1024 * 1024 {
            ...
            eprintln!("⚠️ Large allocation detected: {} MB in component {}", 
                     allocation_info.size_mb(), component);
        }
    ...
    pub fn track_deallocation(&mut self, component: &str, size_bytes: usize) -> RusTorchResult<()> {
        ...
        let allocation_to_remove = self.allocations.iter()
        ...
        self.allocations.values()
            .filter(|info| {
                now.duration_since(info.timestamp)
                    .unwrap_or_default()
                    .as_secs() > self.leak_threshold_seconds
            })
            .cloned()
            .collect()
    }
    ...
    pub fn generate_memory_report(&self) -> RusTorchResult<MemoryReport> {
        ...
        let component_stats: Vec<ComponentMemoryStats> = self.component_stats.values()
            .cloned()
            .collect();
    ...
    pub fn generate_summary_report(&self) -> String {
        ...
        if !report.component_stats.is_empty() {
            ...
            for (i, stats) in sorted_components.iter().take(5).enumerate() {
                summary.push_str(&format!("  {}. {}: {:.2} MB ({} allocations)\n",
                                        i + 1,
...
mod tests {
    ...
    fn test_leak_detection() {
        ...
        tracker.track_allocation("potential_leak", 1024 * 1024).unwrap();
    ...
    fn test_large_allocation_detection() {
        ...
        tracker.track_allocation("large_tensor", 2 * 1024 * 1024).unwrap();
