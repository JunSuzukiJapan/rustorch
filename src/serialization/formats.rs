...
impl SafeTensorsFormat {
    ...
    pub fn load_safetensors<P: AsRef<Path>, T: Float + 'static>(
        ...
        for (name, meta) in header {
            ...
            let data_offsets = meta["data_offsets"].as_array()
                .ok_or_else(|| SerializationError::FormatError("Invalid data_offsets".to_string()))?;
        ...
        T: From<f64>,
    {
        ...
        for (name, tensor) in tensors {
            if let Some(data) = tensor.data.as_slice() {
                let converted_data: Vec<T> = data.iter()
...
mod tests {
    ...
    fn test_huggingface_format() {
        ...
        // Test load
        let (loaded_tensors, loaded_config) = HuggingFaceFormat::load_hf::<_, f32>(&temp_dir).unwrap();
