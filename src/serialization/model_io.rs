...
use super::core::{
    ComputationGraph, FileHeader, Loadable, ModelMetadata, Saveable, SerializationError,
    SerializationResult, TensorMetadata, compute_checksum,
};
...
impl<T: Float + 'static> Saveable for StateDict<T> {
    ...
    fn metadata(&self) -> HashMap<String, String> {
        ...
        meta.insert("training_state".to_string(), self.metadata.training_state.to_string());
        meta
    }
}
...
impl<T: Float + 'static> SafeTensorFormat<T> {
    ...
    pub fn save_safetensors<P: AsRef<Path>>(&self, path: P) -> SerializationResult<()> {
        ...
        for (name, tensor) in &self.tensors {
            ...
            header_data.insert(name.clone(), serde_json::json!({
...
impl<T: Float + 'static> Saveable for ModelCheckpoint<T> {
    ...
    fn metadata(&self) -> HashMap<String, String> {
        ...
        meta.insert("model_type".to_string(), self.model_state.metadata.model_type.clone());
        meta
    }
}
...
impl<T: Float + 'static> Loadable for Tensor<T> {
    fn load_binary(data: &[u8]) -> SerializationResult<Self> {
        ...
        if actual_elements != expected_elements {
            return Err(SerializationError::FormatError(
                format!("Shape/data mismatch: shape requires {} elements, data has {}", 
