...
impl<T: Float + From<f32> + From<u8> + Copy + 'static> Dataset<T> for MNIST<T> {
    ...
    fn get(&self, index: usize) -> Option<(Tensor<T>, Tensor<T>)> {
        ...
        let label = Tensor::from_vec(vec![<T as From<u8>>::from(self.labels[index] as u8)], vec![1]);
...
impl<T: Float + From<f32> + From<u8> + Copy + 'static> Dataset<T> for CIFAR100<T> {
    ...
    fn get(&self, index: usize) -> Option<(Tensor<T>, Tensor<T>)> {
        ...
        // デフォルトで細かいラベルを返す
        let label = Tensor::from_vec(vec![<T as From<u8>>::from(self.fine_labels[index] as u8)], vec![1]);
                ...
                if path.is_file() {
                    if let Some(extension) = path.extension() {
                        if let Some(ext_str) = extension.to_str() {
                            ...
                            // ファイルが画像かどうかをチェック
                            if matches!(ext_str.to_lowercase().as_str(), "jpg" | "jpeg" | "png" | "bmp" | "tiff") {
