...
use crate::nn::{Module, Sequential, Conv2d, MaxPool2d, Linear, BatchNorm2d, Dropout, ReLU, Softmax};
    ...
    T: Float + 'static + Send + Sync + Debug + Default + Copy + From<f32> + ndarray::ScalarOperand + num_traits::FromPrimitive + num_traits::ToPrimitive + num_traits::Zero + num_traits::One + std::iter::Sum + std::fmt::Display,
{
    ...
    pub fn new(
        ...
        for (i, &out_channels) in hidden_channels.iter().enumerate() {
            ...
            features.add_module(Conv2d::new(in_channels, out_channels, (3, 3), Some((1, 1)), Some((1, 1)), Some(true)));
    ...
    T: Float + 'static + Send + Sync + Debug + Default + Copy + From<f32> + ndarray::ScalarOperand + num_traits::FromPrimitive + num_traits::ToPrimitive + num_traits::Zero + num_traits::One + std::iter::Sum + std::fmt::Display,
{
    ...
    fn config(&self) -> HashMap<String, String> {
        ...
        config.insert("feature_layers".to_string(), self.features.len().to_string());
        config.insert("classifier_layers".to_string(), self.classifier.len().to_string());
        config
    }
    ...
    T: Float + 'static + Send + Sync + Debug + Default + Copy + From<f32> + ndarray::ScalarOperand + num_traits::FromPrimitive + num_traits::ToPrimitive + num_traits::Zero + num_traits::One + std::iter::Sum + std::fmt::Display,
{
    ...
    fn build(self) -> Self::Model {
        let input_channels = self.input_channels.expect("Input channels must be specified");
        let num_classes = self.num_classes.expect("Number of classes must be specified");
        
        CNN::new(input_channels, num_classes, self.hidden_channels, self.dropout_rate)
    }
}
    ...
    T: Float + 'static + Send + Sync + Debug + Default + Copy + From<f32> + ndarray::ScalarOperand + num_traits::FromPrimitive + num_traits::ToPrimitive + num_traits::Zero + num_traits::One + std::iter::Sum + std::fmt::Display,
{
    ...
    pub fn new(
        ...
        ResNetBlock {
            conv1: Conv2d::new(in_channels, out_channels, (3, 3), Some((stride, stride)), Some((1, 1)), Some(true)),
            bn1: BatchNorm2d::new(out_channels, None, None, None),
            conv2: Conv2d::new(out_channels, out_channels, (3, 3), Some((1, 1)), Some((1, 1)), Some(true)),
    ...
    pub fn new(
        ...
        for (i, &num_blocks) in layers.iter().enumerate() {
            ...
            let downsample = if stride != 1 || in_channels != out_channels {
                ...
                ds.add_module(Conv2d::new(in_channels, out_channels, (1, 1), Some((stride, stride)), Some((0, 0)), Some(true)));
            ...
            layer.add_module(ResNetBlock::new(in_channels, out_channels, stride, downsample));
