...
impl TensorVisualizer {
        ...
        T: Float + std::fmt::Display + std::fmt::Debug + 'static,
    {
        ...
        if shape.len() != 2 {
            return Err(RusTorchError::InvalidDataFormat(format!(
                "Expected 2D tensor, got {}D tensor",
                shape.len()
            ))
            .into());
        }
        ...
        T: Float + std::fmt::Display + std::fmt::Debug + 'static,
    {
        ...
        if shape.len() != 1 {
            return Err(RusTorchError::InvalidDataFormat(format!(
                "Expected 1D tensor, got {}D tensor",
                shape.len()
            ))
            .into());
        }
        ...
        T: Float + std::fmt::Display + std::fmt::Debug + 'static,
    {
        ...
        if shape.len() != 3 {
            return Err(RusTorchError::InvalidDataFormat(format!(
                "Expected 3D tensor, got {}D tensor",
                shape.len()
            ))
            .into());
        }
...
"#
        ...
        T: Float + std::fmt::Display,
    {
        ...
        for (i, &value) in data.iter().enumerate() {
            ...
            if self.config.show_values {
                let text_color = if value.to_f32().unwrap_or(0.0) > 0.5 {
                    "#fff"
                } else {
                    "#000"
                };
        ...
        T: Float + std::fmt::Display,
    {
        ...
        stats.insert(
            "Range".to_string(),
            (max_val - min_val).to_f32().unwrap_or(0.0),
        );
