...
impl<Q: QuantizableInteger> QuantizedOps<Q> for QuantizedTensor<Q> {
    fn qadd(&self, other: &QuantizedTensor<Q>) -> RusTorchResult<QuantizedTensor<Q>> {
        ...
        let result_data = if self.is_compatible_with(other) {
            ...
            Zip::from(&self.data)
                .and(&other.data)
                .map_collect(|&a, &b| {
                    let sum = QuantizableInteger::to_i32(&a).saturating_add(QuantizableInteger::to_i32(&b)).saturating_sub(self.zero_point);
            ...
            Zip::from(&self.data)
                ...
                .map_collect(|&a, &b| {
                    ...
                    let a_fp = (QuantizableInteger::to_i32(&a) - self.zero_point) as f32 * self.scale;
                    let b_fp = (QuantizableInteger::to_i32(&b) - other.zero_point) as f32 * other.scale;
    ...
    fn qsub(&self, other: &QuantizedTensor<Q>) -> RusTorchResult<QuantizedTensor<Q>> {
        ...
        let result_data = if self.is_compatible_with(other) {
            Zip::from(&self.data)
                .and(&other.data)
                .map_collect(|&a, &b| {
                    let diff = QuantizableInteger::to_i32(&a).saturating_sub(QuantizableInteger::to_i32(&b)).saturating_add(self.zero_point);
    ...
    fn qmul(&self, other: &QuantizedTensor<Q>) -> RusTorchResult<QuantizedTensor<Q>> {
        ...
        let result_data = Zip::from(&self.data)
            ...
            .map_collect(|&a, &b| {
                ...
                // 目標精度にスケールダウン
                let scaled_product = (product as f32 / (self.scale * other.scale / result_scale)).round() as i32;
    ...
    fn qadd_scalar(&self, scalar: f32) -> RusTorchResult<QuantizedTensor<Q>> {
        ...
        let result_data = self.data.mapv(|val| {
            let sum = QuantizableInteger::to_i32(&val).saturating_add(QuantizableInteger::to_i32(&scalar_clamped)).saturating_sub(self.zero_point);
            Q::from_i32_clamped(sum)
        });
...
pub trait DequantizeOps<Q: QuantizableInteger> {
    ...
    /// 混合精度演算のための部分非量子化
    fn dequantize_partial(&self, new_scale: f32, new_zero_point: i32) -> RusTorchResult<QuantizedTensor<Q>>;
}
...
fn compute_output_quantization_params(
    ...
    match operation {
        ...
        QuantizedOperation::Div => {
            ...
            // 除算の場合、スケールは入力スケールの比
            let result_scale = if scale2 != 0.0 { scale1 / scale2 } else { scale1 };
            let result_zero_point = 0;
...
mod tests {
    ...
    fn test_quantized_addition() {
        let data1 = Array2::from_shape_vec((2, 2), vec![10i8, 20, 30, 40]).unwrap().into_dyn();
        let data2 = Array2::from_shape_vec((2, 2), vec![5i8, 10, 15, 20]).unwrap().into_dyn();
    ...
    fn test_quantized_multiplication() {
        let data1 = Array2::from_shape_vec((2, 2), vec![2i8, 3, 4, 5]).unwrap().into_dyn();
        let data2 = Array2::from_shape_vec((2, 2), vec![3i8, 4, 5, 6]).unwrap().into_dyn();
    ...
    fn test_quantized_relu() {
        let data = Array2::from_shape_vec((2, 2), vec![-10i8, -5, 5, 10]).unwrap().into_dyn();
    ...
    fn test_quantized_matmul() {
        let data1 = Array2::from_shape_vec((2, 3), vec![1i8, 2, 3, 4, 5, 6]).unwrap().into_dyn();
        let data2 = Array2::from_shape_vec((3, 2), vec![7i8, 8, 9, 10, 11, 12]).unwrap().into_dyn();
    ...
    #[test]
    fn test_qlinear() {
        let input_data = Array2::from_shape_vec((1, 3), vec![1i8, 2, 3]).unwrap().into_dyn();
        let weight_data = Array2::from_shape_vec((3, 2), vec![1i8, 2, 3, 4, 5, 6]).unwrap().into_dyn();
