...
#[cfg(feature = "metal")]
use metal::{
    Device, CommandQueue, Library, ComputePipelineState, Buffer,
    MTLResourceOptions, MTLSize, CommandBuffer, ComputeCommandEncoder,
};
#[cfg(feature = "metal")]
...
impl MetalKernelExecutor {
    ...
    pub fn reduce_sum_f32(&self, input: &[f32]) -> Result<f32, GpuError> {
        ...
        compute_encoder.set_threadgroup_memory_length(0, (block_size * std::mem::size_of::<f32>()) as u64);
...
/// Metal要素ごと加算を実行
pub fn metal_elementwise_add_f32(
    _a: &[f32],
    _b: &[f32],
    _c: &mut [f32],
) -> Result<(), GpuError> {
