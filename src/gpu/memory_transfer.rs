...
impl<T: Float + 'static> GpuMemoryManager<T> {
        ...
        F: Fn(T, T) -> T + Send + Sync,
    {
        match (lhs, rhs) {
            #[cfg(feature = "cuda")]
            (GpuBuffer::Cuda { data: lhs_data, device }, GpuBuffer::Cuda { data: rhs_data, .. }) => {
                self.execute_cuda_elementwise(lhs_data, rhs_data, device, &op)
            }
            
            #[cfg(feature = "metal")]
            (GpuBuffer::Metal { buffer: lhs_buf, device }, GpuBuffer::Metal { buffer: rhs_buf, .. }) => {
                self.execute_metal_elementwise(lhs_buf, rhs_buf, device, &op)
            }
            
            #[cfg(feature = "opencl")]
            (GpuBuffer::OpenCL { buffer: lhs_buf, context }, GpuBuffer::OpenCL { buffer: rhs_buf, .. }) => {
        ...
        F: Fn(T, T) -> T + Send + Sync,
    {
        let lhs_data = match lhs {
            ...
            #[cfg(feature = "cuda")]
            GpuBuffer::Cuda { .. } => return Err(RusTorchError::gpu("CPU fallback requires CPU buffers")),
            #[cfg(feature = "metal")]
            GpuBuffer::Metal { .. } => return Err(RusTorchError::gpu("CPU fallback requires CPU buffers")),
            #[cfg(feature = "opencl")]
            GpuBuffer::OpenCL { .. } => return Err(RusTorchError::gpu("CPU fallback requires CPU buffers")),
        };
    ...
    pub fn execute_batch_normalize(
        ...
        match tensor {
            ...
            // CPU fallback
            GpuBuffer::Cpu(data) => {
                self.execute_cpu_batch_normalize(data, epsilon)
            }
        }
    }
    ...
    fn execute_cpu_batch_normalize(
        ...
        let normalized: Vec<T> = input_data.iter()
    ...
    pub fn execute_attention(
        ...
        match (query, key, value) {
            #[cfg(feature = "cuda")]
            (
                GpuBuffer::Cuda { data: query_data, device },
                GpuBuffer::Cuda { data: key_data, .. },
                GpuBuffer::Cuda { data: value_data, .. }
            ) => {
            ...
            #[cfg(feature = "metal")]
            (
                GpuBuffer::Metal { buffer: query_buf, device },
                GpuBuffer::Metal { buffer: key_buf, .. },
                GpuBuffer::Metal { buffer: value_buf, .. }
            ) => {
            ...
            #[cfg(feature = "opencl")]
            (
                GpuBuffer::OpenCL { buffer: query_buf, context },
                GpuBuffer::OpenCL { buffer: key_buf, .. },
                GpuBuffer::OpenCL { buffer: value_buf, .. }
            ) => {
    ...
    fn execute_cpu_attention(
        ...
        let query_data = match query {
            GpuBuffer::Cpu(data) => data.as_slice(),
            #[cfg(feature = "cuda")]
            GpuBuffer::Cuda { .. } => return Err(RusTorchError::gpu("CPU attention requires CPU buffers")),
            #[cfg(feature = "metal")]
            GpuBuffer::Metal { .. } => return Err(RusTorchError::gpu("CPU attention requires CPU buffers")),
            #[cfg(feature = "opencl")]
            GpuBuffer::OpenCL { .. } => return Err(RusTorchError::gpu("CPU attention requires CPU buffers")),
        };
        ...
        // scores = query @ key^T
        let scores: Vec<T> = query_data.iter()
            .zip(key_data.iter())
            .map(|(&q, &k)| q * k) // 簡単なドット積近似
            .collect();
        
        // softmax適用（簡単版）
        let max_score = scores.iter().fold(T::neg_infinity(), |max, &x| {
            if x > max { x } else { max }
        });
        
        let exp_scores: Vec<T> = scores.iter()
            .map(|&x| (x - max_score).exp())
            .collect();
            
        let sum_exp = exp_scores.iter().fold(T::zero(), |acc, &x| acc + x);
        
        let attention_weights: Vec<T> = exp_scores.iter()
            .map(|&x| x / sum_exp)
            .collect();
        
        // 重み付きvalue計算
        let result: Vec<T> = attention_weights.iter()
            .zip(value_data.iter())
            .map(|(&w, &v)| w * v)
            .collect();
        ...
        T: cudarc::driver::DeviceRepr,
    {
        ...
        unsafe {
            ...
            if let Ok(ptx) = cudarc::nvrtc::compile_ptx(kernel_src) {
                if device.load_ptx(ptx, "elementwise", &["elementwise_add_f32"]).is_ok() {
...
impl<T: Float + 'static> GpuMemoryManager<T> {
        ...
        F: Fn(T, T) -> T + Send + Sync,
    {
        ...
        let lhs_cpu = Self::metal_transfer_to_cpu(lhs, &[lhs.length() as usize / std::mem::size_of::<T>()])?;
        let rhs_cpu = Self::metal_transfer_to_cpu(rhs, &[rhs.length() as usize / std::mem::size_of::<T>()])?;
    ...
    fn execute_metal_batch_normalize(
        ...
        let cpu_data = Self::metal_transfer_to_cpu(buffer, &[buffer.length() as usize / std::mem::size_of::<T>()])?;
        let gpu_manager = GpuMemoryManager::new();
        let cpu_buffer = GpuBuffer::Cpu(Arc::new(cpu_data));
        let result = gpu_manager.execute_cpu_batch_normalize(
            match &cpu_buffer {
    ...
    fn execute_metal_attention(
        ...
        let query_cpu = Self::metal_transfer_to_cpu(query, &[query.length() as usize / std::mem::size_of::<T>()])?;
        let key_cpu = Self::metal_transfer_to_cpu(key, &[key.length() as usize / std::mem::size_of::<T>()])?;
        let value_cpu = Self::metal_transfer_to_cpu(value, &[value.length() as usize / std::mem::size_of::<T>()])?;
