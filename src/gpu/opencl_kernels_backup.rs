...
pub mod opencl_utils {
    ...
    /// カーネルソースコードを取得
    pub fn get_kernel_source() -> String {
        include_str!("opencl_kernels.cl").to_string()
    }
    
    /// Get kernel name for kernel type
    /// カーネルタイプのカーネル名を取得
    pub fn get_kernel_name(kernel_type: OpenClKernelType) -> std::ffi::CString {
        let name = match kernel_type {
            ...
            OpenClKernelType::Convolution => "convolution_2d",
    ...
    /// 最適なワークグループサイズを計算
    pub fn calculate_work_group_size(total_size: usize, device_max_work_group_size: usize) -> usize {
        let preferred_sizes = [256, 128, 64, 32, 16];
        
        for &size in &preferred_sizes {
            if size <= device_max_work_group_size && total_size % size == 0 {
                return size;
            }
        }
        
        // Fallback to device maximum or 1
        std::cmp::min(device_max_work_group_size, total_size).max(1)
    }
}
