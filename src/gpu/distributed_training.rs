...
use crate::gpu::multi_gpu::{MultiGpuContext, ParallelismStrategy, GradientCompression};
use crate::gpu::sync_primitives::{StreamManager, MultiGpuBarrier, StreamPriority};
...
pub enum LearningRateSchedule {
    ...
    Constant(f32),
    /// Linear decay
    LinearDecay { initial: f32, final_rate: f32, steps: usize },
    /// Exponential decay
    ExponentialDecay { initial: f32, decay_rate: f32, decay_steps: usize },
    /// Cosine annealing
    CosineAnnealing { initial: f32, min_rate: f32, period: usize },
    /// Warm-up followed by decay
    WarmupDecay { warmup_steps: usize, initial: f32, decay_rate: f32 },
}
...
impl DistributedTrainer {
    ...
    fn get_current_learning_rate(&self) -> f32 {
        ...
        match &self.config.lr_schedule {
            ...
            LearningRateSchedule::LinearDecay { initial, final_rate, steps } => {
            ...
            LearningRateSchedule::ExponentialDecay { initial, decay_rate, decay_steps } => {
                let decay_factor = (current_step / decay_steps) as f32;
                initial * decay_rate.powf(decay_factor)
            }
            LearningRateSchedule::CosineAnnealing { initial, min_rate, period } => {
            ...
            LearningRateSchedule::WarmupDecay { warmup_steps, initial, decay_rate } => {
...
impl TrainingMetrics {
    ...
    pub fn efficiency_ratio(&self) -> f32 {
        if self.total_time.as_secs_f32() > 0.0 {
            let compute_time = self.total_time.as_secs_f32() - self.communication_overhead.as_secs_f32();
...
mod tests {
    ...
    fn test_learning_rate_schedule() {
        ...
        let trainer = DistributedTrainer::new(vec![0], ParallelismStrategy::DataParallel, config).unwrap();
