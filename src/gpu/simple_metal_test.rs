...
pub fn test_metal_gpu_basic() -> RusTorchResult<()> {
    #[cfg(all(target_os = "macos", feature = "metal"))]
    {
        ...
        if let Some(device) = Device::system_default() {
            ...
            println!("✅ Metal buffer created successfully: {} bytes", buffer.length());
            
            // Test buffer read-back
            let contents = buffer.contents();
            let read_data = unsafe {
                std::slice::from_raw_parts(contents as *const f32, data.len())
            };
...
pub fn test_metal_tensor_operations() -> RusTorchResult<()> {
    ...
    println!("✅ CPU tensor addition successful: {:?}", cpu_result.data.as_slice());
...
pub fn benchmark_metal_performance() -> RusTorchResult<()> {
    ...
    for size in sizes {
        ...
        let cpu_result = matrix_a.matmul(&matrix_b).map_err(|e| RusTorchError::gpu(&e))?;
        ...
        println!("  First few elements: {:?}", &cpu_result.data.as_slice().unwrap()[..4.min(cpu_result.data.len())]);
    }
