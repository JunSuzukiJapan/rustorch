...
impl HardwareOptimizer {
    ...
    fn detect_cpu_extensions() -> Vec<String> {
        ...
        #[cfg(target_arch = "x86_64")]
        {
            if is_x86_feature_detected!("sse") { extensions.push("SSE".to_string()); }
            if is_x86_feature_detected!("sse2") { extensions.push("SSE2".to_string()); }
            if is_x86_feature_detected!("sse3") { extensions.push("SSE3".to_string()); }
            if is_x86_feature_detected!("ssse3") { extensions.push("SSSE3".to_string()); }
            if is_x86_feature_detected!("sse4.1") { extensions.push("SSE4.1".to_string()); }
            if is_x86_feature_detected!("sse4.2") { extensions.push("SSE4.2".to_string()); }
            if is_x86_feature_detected!("avx") { extensions.push("AVX".to_string()); }
            ...
            if is_x86_feature_detected!("fma") { extensions.push("FMA".to_string()); }
        }
...
mod tests {
    ...
    fn test_hardware_detection() {
        ...
        println!("\nMemory Hierarchy:");
        println!("  L1 cache: {} KB", capabilities.memory_hierarchy.l1_cache / 1024);
        println!("  L2 cache: {} KB", capabilities.memory_hierarchy.l2_cache / 1024);
        if let Some(l3) = capabilities.memory_hierarchy.l3_cache {
            println!("  L3 cache: {} MB", l3 / (1024 * 1024));
        }
        
        println!("\nAccelerators: {} detected", capabilities.accelerators.len());
