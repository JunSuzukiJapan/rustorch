...
impl<T: Float + 'static + ndarray::ScalarOperand + num_traits::FromPrimitive + std::cmp::PartialOrd + Clone> Tensor<T> {
    ...
    pub fn topk(&self, k: usize) -> RusTorchResult<(Tensor<T>, Vec<usize>)> {
        if k > self.numel() {
            return Err(RusTorchError::InvalidOperation {
                ...
                message: format!("k ({}) cannot be larger than tensor size ({})", k, self.numel()),
            });
        }

        let mut indexed_data: Vec<(usize, T)> = self.data
            .iter()
            .enumerate()
            .map(|(idx, &val)| (idx, val))
            .collect();
        ...
        indexed_data.sort_by(|(_, a), (_, b)| b.partial_cmp(a).unwrap_or(std::cmp::Ordering::Equal));
    ...
    pub fn bottomk(&self, k: usize) -> RusTorchResult<(Tensor<T>, Vec<usize>)> {
        ...
        indexed_data.sort_by(|(_, a), (_, b)| a.partial_cmp(b).unwrap_or(std::cmp::Ordering::Equal));
