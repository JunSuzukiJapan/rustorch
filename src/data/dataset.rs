...
impl<T: Float + 'static> TensorDataset<T> {
    ...
    pub fn from_features_targets(features: Vec<Tensor<T>>, targets: Vec<Tensor<T>>) -> Result<Self, DataError> {
    ...
    fn get_item(&self, index: usize) -> Result<Vec<Tensor<T>>, DataError> {
        if index >= self.len() {
            return Err(RusTorchError::InvalidParameters {
                ...
                message: format!("Index {} out of bounds for dataset of size {}", index, self.len()),
            });
        }
