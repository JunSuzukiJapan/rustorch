...
mod tests {
    ...
    fn test_dataloader_performance_comparison() {
        ...
        assert!(
            eager_duration.as_millis() < 1000,
            "Eager loading took too long: {:?}",
            eager_duration
        );
        assert!(
            lazy_duration.as_millis() < 1000,
            "Lazy loading took too long: {:?}",
            lazy_duration
        );

        println!("Performance comparison:");
        println!(
            "  Eager loading: {:?} for {} batches",
            eager_duration, eager_batches
        );
        println!(
            "  Lazy loading:  {:?} for {} batches",
            lazy_duration, lazy_batches
        );
    }
    ...
    fn test_dataloader_with_transformations_and_lazy_loading() {
        ...
        let transforms: Vec<Box<dyn Transform<f32> + Send + Sync>> =
            vec![Box::new(normalize), Box::new(flip)];
