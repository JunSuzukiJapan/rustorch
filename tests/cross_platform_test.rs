...
use rustorch::optimization::{
    SimdOptimizer, SimdBackend, VectorizedOperation,
    PlatformOptimizer, OptimizationLevel,
    HardwareOptimizer,
};
...
fn test_cross_platform_consistency() {
    ...
    // Addition
    let scalar_result = scalar_opt.apply_vectorized(
        VectorizedOperation::Add,
        &a,
        Some(&b)
    ).unwrap();
    
    let auto_result = auto_opt.apply_vectorized(
        VectorizedOperation::Add,
        &a,
        Some(&b)
    ).unwrap();
