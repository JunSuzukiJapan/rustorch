...
use rustorch::optim::{Optimizer, LAMB, AdaBound, LBFGS, LineSearchMethod};
...
fn test_optimizer_performance_comparison() {
    ...
    let initial_param = param.clone();
    let mut param_lamb = initial_param.clone();
    let mut param_adabound = initial_param.clone();
    let mut param_lbfgs = initial_param.clone();
    ...
    let initial_sum_squares = initial_param.data.iter().map(|&x| x * x).sum::<f32>().sqrt();
    let lamb_sum_squares = param_lamb.data.iter().map(|&x| x * x).sum::<f32>().sqrt();
    let adabound_sum_squares = param_adabound.data.iter().map(|&x| x * x).sum::<f32>().sqrt();
