...
fn test_tensor_save_load_roundtrip() {
    let original_tensor = Tensor::from_vec(
    ...
    assert_eq!(original_tensor.data.as_slice(), loaded_tensor.data.as_slice());
...
fn test_model_checkpoint_system() {
    ...
    // Save checkpoint
    checkpoint.save_checkpoint(&temp_path).expect("Failed to save checkpoint");
    
    // Load checkpoint
    let loaded_checkpoint = ModelCheckpoint::<f32>::load_checkpoint(&temp_path)
        .expect("Failed to load checkpoint");
    
    // Verify checkpoint data
    assert_eq!(loaded_checkpoint.epoch, 10);
    assert_eq!(loaded_checkpoint.step, 1000);
    assert!(loaded_checkpoint.model_state.get_parameter("fc.weight").is_some());
...
fn test_jit_script_module() {
    ...
    let loaded_module: ScriptModule<f32> = load(&temp_path)
...
fn test_jit_trace_functionality() {
    ...
    let traced_module = trace(|inputs: &[Tensor<f32>]| {
        if inputs.is_empty() {
            vec![]
        } else {
            let mut result = inputs[0].clone();
    ...
    }, &inputs).expect("Failed to trace function");
    ...
    // Test execution
    let output = traced_module.forward(&inputs).expect("Failed to execute traced module");
...
fn test_safetensors_format() {
    ...
    SafeTensorsFormat::save_safetensors(&tensors, &temp_path)
...
fn test_huggingface_format() {
    ...
    let (loaded_tensors, loaded_config) = HuggingFaceFormat::load_hf::<_, f32>(&temp_dir)
        .expect("Failed to load HuggingFace format");
...
fn test_format_detection() {
    ...
    // Test format detection
    let detected_format = detect_format(&rustorch_path)
...
fn test_jit_cache_system() {
    ...
    let mut cache = JitCache::<f32>::new(&cache_dir)
        .expect("Failed to create JIT cache");
    
    // Create a module
    let mut module = ScriptModule::<f32>::new();
    let tensor = Tensor::from_vec(vec![1.0, 2.0], vec![2]);
    module.add_parameter("test_param".to_string(), tensor);
    
    // Store in cache
    cache.store("test_module".to_string(), module)
        .expect("Failed to store module in cache");
    
    // Retrieve from cache
    let cached_module = cache.get("test_module")
        .expect("Failed to retrieve module from cache");
    assert!(cached_module.parameters.contains_key("test_param"));
    
    // Test cache persistence
    let mut new_cache = JitCache::<f32>::new(&cache_dir)
        .expect("Failed to create new cache instance");
    let persisted_module = new_cache.get("test_module")
...
fn test_precision_conversion() {
    ...
    let tensor_f32 = Tensor::from_vec(
        vec![1.0f32, 2.5, 3.7, -1.2], 
        vec![2, 2]
    );
    tensors_f32.insert("test_tensor".to_string(), tensor_f32);
    
    // Convert f32 to f64
    let tensors_f64: HashMap<String, Tensor<f64>> = 
        conversion::convert_precision(&tensors_f32);
...
fn test_large_tensor_serialization() {
    ...
    safe_format.metadata.insert("description".to_string(), "test large tensor".to_string());
    
    let temp_path = std::env::temp_dir().join("large_tensor.safetensors");
    
    // Save and verify
    safe_format.save_safetensors(&temp_path)
...
fn test_graph_execution() {
    ...
    let outputs = module.forward(&[input_tensor])
