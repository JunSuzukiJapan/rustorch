...
use rustorch::gpu::multi_gpu::{MultiGpuContext, ParallelismStrategy, GpuTopology};
...
fn test_multi_gpu_barrier_basic() {
    ...
    assert!(result.is_ok(), "Single GPU barrier should succeed immediately");
...
fn test_stream_manager_basic() -> RusTorchResult<()> {
    ...
    // Test event query
    assert!(manager.query_event(event_id), "Event should be completed after recording");
...
fn test_parallelism_strategies() {
    ...
    for strategy in strategies {
        ...
        assert!(context_result.is_ok(), "Strategy {:?} should be supported", strategy);
    }
...
fn run_multi_gpu_basic_validation_suite() {
    ...
    let test_results: Vec<(&str, Box<dyn Fn() -> Result<(), Box<dyn std::error::Error>>>)> = vec![
        ("GPU Topology", Box::new(|| {
            test_gpu_topology_creation();
            Ok(())
        })),
        ("Multi-GPU Context", Box::new(|| {
            test_multi_gpu_context_basic()?;
            Ok(())
        })),
        ("Stream Manager", Box::new(|| {
            test_stream_manager_basic()?;
            Ok(())
        })),
        ("Distributed Trainer", Box::new(|| {
            test_distributed_trainer_basic()?;
            Ok(())
        })),
        ("Tensor Compatibility", Box::new(|| {
            test_tensor_multi_gpu_compatibility()?;
            Ok(())
        })),
        ("P2P Communication", Box::new(|| {
            test_p2p_communication_basic()?;
            Ok(())
        })),
        ("Stream Priorities", Box::new(|| {
    ...
    println!("=== Multi-GPU Infrastructure: {} ===", 
             if passed == total { "VALIDATED ✅" } else { "NEEDS ATTENTION ❌" });
}