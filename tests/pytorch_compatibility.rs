...
use rustorch::nn::{Linear, Conv2d, BatchNorm2d, ReLU};
use rustorch::optim::{SGD, Adam, RMSprop, AdaGrad, Optimizer};
...
mod pytorch_compatibility_tests {
    ...
    fn test_autograd_compatibility() {
        ...
        println!("  ✓ Forward pass: z = x*y + x^2 = {}", 
        ...
        if let (Some(ref x_g), Some(ref y_g)) = (x_grad.as_ref(), y_grad.as_ref()) {
            ...
            println!("    dz/dx = {} (expected: y + 2*x = 3 + 2*2 = 7)", x_grad_val);
            println!("    dz/dy = {} (expected: x = 2)", y_grad_val);
            
            // Verify gradient values (allowing for small floating point errors)
            assert!((x_grad_val - 7.0_f32).abs() < 1e-6, "x gradient should be 7.0");
            assert!((y_grad_val - 2.0_f32).abs() < 1e-6, "y gradient should be 2.0");
        } else {
    ...
    fn test_model_format_concepts() {
        ...
        println!("  ✓ Sample weight tensor created: {:?}", sample_weights.shape());
