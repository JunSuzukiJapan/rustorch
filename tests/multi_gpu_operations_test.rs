...
use rustorch::gpu::distributed_training::{DistributedTrainer, TrainingConfig, CompressionType};
#[cfg(all(
    not(target_arch = "wasm32"),
    not(target_os = "macos"),
    not(target_os = "linux"),
...
mod communication_tests {
    ...
    fn test_multi_gpu_context_creation() -> RusTorchResult<()> {
        ...
        println!("Multi-GPU context created successfully with {} GPUs", context.gpu_count());
        Ok(())
    }
    ...
    fn test_all_reduce_operations() -> RusTorchResult<()> {
        ...
        for (actual, expected) in nccl_result[0].data.iter().zip(expected.iter()) {
            assert!((actual - expected).abs() < 1e-5, 
                   "All-reduce result {} doesn't match expected {}", actual, expected);
        }
...
mod synchronization_tests {
    ...
    fn test_stream_manager() -> RusTorchResult<()> {
        ...
        // Query event status
        assert!(manager.query_event(event1), "Event should be completed after recording");
        assert!(manager.query_event(event2), "Event should be completed after recording");
...
mod distributed_training_tests {
    ...
    fn test_gradient_synchronization() -> RusTorchResult<()> {
        ...
        let gradients = vec![("weight1".to_string(), grad1), ("weight2".to_string(), grad2)];
...
mod performance_tests {
    ...
    fn test_all_reduce_performance() -> RusTorchResult<()> {
        ...
        println!("All-reduce performance test: {:?} for {} elements", duration, size);
    ...
    fn test_distributed_training_performance() -> RusTorchResult<()> {
        ...
        println!("Distributed training sync time: {:?} for {} gradient elements", 
                sync_duration, grad_size * 2);
...
mod fault_tolerance_tests {
    ...
    fn test_barrier_timeout() {
        ...
        let result = barrier.wait(0);
        assert!(result.is_err(), "Barrier should timeout when not all GPUs reach it");
    ...
    fn test_invalid_gpu_id_handling() {
        ...
        if let Ok(context) = context_result {
            assert!(!context.is_gpu_available(999), "Invalid GPU ID should return false");
    ...
    fn test_tensor_shape_mismatch_error() -> RusTorchResult<()> {
        ...
        let result = context.all_reduce(tensors);
        assert!(result.is_err(), "All-reduce should fail with mismatched tensor shapes");
...
mod stream_management_tests {
    ...
    fn test_event_wait_with_timeout() -> RusTorchResult<()> {
        ...
        assert!(result.is_ok(), "Wait on completed event should succeed immediately");
        ...
        assert!(timeout_result.is_err(), "Wait on unrecorded event should timeout");
