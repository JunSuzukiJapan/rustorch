...
use rustorch::gpu::multi_gpu::{ParallelismStrategy, GradientCompression};
...
fn test_distributed_trainer_profiling_integration() -> RusTorchResult<()> {
    ...
    assert!(report.session_duration > Duration::ZERO, "Session should have non-zero duration");
...
fn test_rustorch_profiler_integration() -> RusTorchResult<()> {
    ...
    assert!(report.operation_summary.operations.contains_key("tensor_multiply"));
    assert!(report.operation_summary.operations.contains_key("tensor_add"));
...
fn test_automated_performance_testing() -> RusTorchResult<()> {
    ...
    assert!(matches!(report.memory_analysis.memory_trend, MemoryTrend::Increasing));
...
fn test_profiler_recommendations() -> RusTorchResult<()> {
    ...
    let slow_op_recommendation = report.recommendations.iter()
        .any(|r| r.contains("slow_tensor_operation") && r.contains("optimization"));
    assert!(slow_op_recommendation, "Should recommend optimizing slow operation");
    
    let memory_recommendation = report.recommendations.iter()
        .any(|r| r.contains("Memory") && r.contains("increasing"));
    assert!(memory_recommendation, "Should recommend checking memory usage");
