...
mod tests {
    ...
    fn test_pipeline_data_transforms() {
        ...
        let normalize = rustorch::wasm::data_transforms::WasmNormalize::new(
            &[0.5], &[0.5]
        ).expect("Failed to create normalize transform");
        
        // Apply transformation
        let result = normalize.apply(&tensor).expect("Failed to apply normalization");
        ...
        for (i, &value) in result_data.iter().enumerate() {
            ...
            assert!((value - expected).abs() < 1e-6, "Normalization failed at index {}", i);
        }
    }
    ...
    fn test_pipeline_multiple_transforms() {
        ...
        let resize = rustorch::wasm::data_transforms::WasmResize::new(
            2, 2, "nearest"
        ...
        let final_result = normalize.apply(&resized).expect("Failed to apply normalization");
    ...
    fn test_advanced_math_integration() {
        ...
        for i in 0..4 {
            ...
            assert!((expected_tanh - actual_tanh).abs() < 1e-6, 
                "tanh relationship failed at index {}", i);
        }
    }
    ...
    fn test_quality_metrics_integration() {
        ...
        // Test completeness
        let clean_completeness = metrics.completeness(&clean_tensor)
            .expect("Failed to calculate clean completeness");
        let dirty_completeness = metrics.completeness(&dirty_tensor)
            .expect("Failed to calculate dirty completeness");
        
        assert!((clean_completeness - 100.0).abs() < 1e-6, "Clean data should be 100% complete");
        assert!(dirty_completeness >= 60.0 && dirty_completeness <= 80.0, 
               "Dirty data completeness should be 60-80%, got {:.2}%", dirty_completeness);
        
        // Test validity
        let clean_validity = metrics.validity(&clean_tensor)
            .expect("Failed to calculate clean validity");
        let dirty_validity = metrics.validity(&dirty_tensor)
            .expect("Failed to calculate dirty validity");
        
        assert!((clean_validity - 100.0).abs() < 1e-6, "Clean data should be 100% valid");
        assert!(dirty_validity >= 60.0 && dirty_validity <= 80.0,
               "Dirty data validity should be 60-80%, got {:.2}%", dirty_validity);
    }
    ...
    fn test_anomaly_detection_integration() {
        ...
        let normal_anomalies = detector.detect_statistical(&normal_tensor)
            .expect("Failed to detect on normal data");
        assert_eq!(normal_anomalies.length(), 0, "Normal data should have no anomalies");
        
        // Test data with anomaly (should find 1 anomaly)
        let found_anomalies = detector.detect_statistical(&anomaly_tensor)
            .expect("Failed to detect on anomaly data");
        assert!(found_anomalies.length() >= 1, "Should detect at least 1 anomaly, found {}", found_anomalies.length());
    }
    ...
    fn test_trait_polymorphism() {
        ...
        let stats_result = stats_analyzer.analyze(&tensor)
            .expect("Stats analysis failed");
        let quality_result = quality_analyzer.analyze(&tensor)
            .expect("Quality analysis failed");
        
        // Verify both return valid JSON
        assert!(stats_result.contains("mean"), "Stats result should contain mean");
        assert!(quality_result.contains("completeness"), "Quality result should contain completeness");
