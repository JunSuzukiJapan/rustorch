...
# Navigate to RusTorch project directory
RUSTORCH_PROJECT_DIR="$project_path"

echo "🚀 RusTorch Jupyter Launcher"
echo "🚀 RusTorch Jupyterランチャー"
echo ""
echo "📍 Project directory: \$RUSTORCH_PROJECT_DIR"
echo "📍 プロジェクトディレクトリ: \$RUSTORCH_PROJECT_DIR"

# Check if project directory exists
if [[ ! -d "\$RUSTORCH_PROJECT_DIR" ]]; then
    echo "❌ RusTorch project directory not found: \$RUSTORCH_PROJECT_DIR"
    echo "❌ RusTorchプロジェクトディレクトリが見つかりません: \$RUSTORCH_PROJECT_DIR"
    echo ""
...
cd "\$RUSTORCH_PROJECT_DIR" || {
    echo "❌ Failed to navigate to \$RUSTORCH_PROJECT_DIR"
    echo "❌ \$RUSTORCH_PROJECT_DIR への移動に失敗しました"
    exit 1
}

# Verify this is a RusTorch project
if [[ ! -f "Cargo.toml" ]] || ! grep -q "rustorch" "Cargo.toml" 2>/dev/null; then
    echo "❌ Invalid RusTorch project directory: \$RUSTORCH_PROJECT_DIR"
    echo "❌ 無効なRusTorchプロジェクトディレクトリ: \$RUSTORCH_PROJECT_DIR"
    echo ""
    echo "🔄 Please reinstall using:"
    echo "🔄 再インストールしてください:"
    echo "   curl -sSL https://raw.githubusercontent.com/JunSuzukiJapan/rustorch/main/install_jupyter.sh | bash"
...
echo "✅ RusTorch project verified"
echo "✅ RusTorchプロジェクトを確認しました"
echo ""
...
if [[ -f "./start_jupyter_quick.sh" ]]; then
    ...
    echo "❌ start_jupyter_quick.sh not found in project directory"
    echo "❌ プロジェクトディレクトリにstart_jupyter_quick.shが見つかりません"
    echo ""
    echo "🔄 Please update your RusTorch installation:"
    echo "🔄 RusTorchインストールを更新してください:"
    echo "   cd \$RUSTORCH_PROJECT_DIR && git pull"
    exit 1
fi
