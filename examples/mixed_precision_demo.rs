...
use rustorch::amp::{
    autocast, AMPConfig, AMPOptimizer, GradScaler, ParamGroup,
    maybe_autocast_f32, cast_to_fp16, cast_to_fp32
};
use rustorch::optim::{sgd::SGD, adamw::AdamW};
...
fn gradient_scaler_demo() -> Result<(), Box<dyn std::error::Error>> {
    ...
    let overflow_grads = vec![
        Tensor::from_vec(vec![1e10, 2e10, 3e10], vec![3]),
    ];
    ...
    println!("   - Scaler stats: enabled={}, scale={}", stats.enabled, stats.current_scale);
...
fn amp_optimizer_demo() -> Result<(), Box<dyn std::error::Error>> {
    ...
    println!("   - Training stats: total_steps={}, success_rate={:.3}", 
             stats.total_steps, stats.success_rate());
...
fn dtype_conversion_demo() -> Result<(), Box<dyn std::error::Error>> {
    ...
    println!("   - Restored FP32 tensor: {:?}", restored_tensor.as_slice());
    ...
    println!("   - Memory footprint FP16 (simulated): {} bytes", memory_fp16_sim);
...
fn complete_training_demo() -> Result<(), Box<dyn std::error::Error>> {
    ...
    for epoch in 0..10 {
        ...
        let current_lr = initial_lr * 0.5 * (1.0 + (std::f32::consts::PI * epoch as f32 / total_epochs as f32).cos());
        
        if epoch % 3 == 0 {
            println!("     Epoch {}: loss={:.4}, lr={:.6}, step={:?}", 
                     epoch, loss_val, current_lr, 
                     match step_result {
    ...
    println!("     * Overflow rate: {:.1}%", final_stats.overflow_rate * 100.0);
