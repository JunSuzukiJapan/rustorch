...
fn demo_matrix_multiplication() -> Result<(), Box<dyn std::error::Error>> {
    ...
    for device in &[DeviceType::Cpu] {
        ...
        let correct = a.iter().zip(c.iter()).all(|(expected, actual)| {
...
fn performance_comparison() -> Result<(), Box<dyn std::error::Error>> {
    ...
    for size in sizes {
        ...
        for device in &[DeviceType::Cpu] {
            ...
            println!("  {}: {:.2}ms avg, {:.1} Melem/s", 
                device, avg_time * 1000.0, throughput);
        }
        println!();
    }
...
fn demo_cuda_specific() -> Result<(), Box<dyn std::error::Error>> {
    use rustorch::gpu::cuda_kernels::{CudaKernelExecutor, cuda_elementwise_add_f32};
...
fn demo_metal_specific() -> Result<(), Box<dyn std::error::Error>> {
    use rustorch::gpu::metal_kernels::{MetalKernelExecutor, metal_elementwise_add_f32};
    ...
    if let Ok(executor) = MetalKernelExecutor::new() {
        ...
        println!("  Metal Direct Call: {} ({:.2}ms)", 
            if correct { "✓" } else { "✗" },
            elapsed.as_secs_f64() * 1000.0
        );
    } else {
