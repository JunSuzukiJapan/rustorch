...
fn main() -> rustorch::error::RusTorchResult<()> {
    ...
    println!("🔥 Fixed Test - {}x{} matrix, {} iterations", test_size, test_size, iterations);
    ...
    // 演算定義
    let perform_f32_operations = |a: &F32Tensor, b: &F32Tensor, executor: &mut F32HybridExecutor| -> rustorch::error::RusTorchResult<f64> {
    ...
    let perform_f64_operations = |a: &Tensor<f64>, b: &Tensor<f64>| -> rustorch::error::RusTorchResult<f64> {
        let start = Instant::now();
    ...
    let metal_time = perform_f32_operations(&matrix_a_f32, &matrix_b_f32, &mut hybrid_f32_executor)?;
    ...
    let existing_time = perform_existing_hybrid_operations(&matrix_a_f64, &matrix_b_f64, &existing_hybrid_executor)?;
    ...
    // 📊 詳細分析
    println!("\n📊 Fixed Hybrid Analysis for {}x{} matrix:", test_size, test_size);
    ...
    println!("\n🔬 GPU Mode Comparison:");
    println!("  True Existing Hybrid vs Metal GPU: {:.2}x ratio", existing_vs_metal);
    println!("  Hybrid_f32 vs Metal GPU:          {:.2}x ratio", f32_vs_metal);
