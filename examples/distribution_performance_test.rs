...
fn main() {
    ...
    test_distribution_sampling("Gamma (Exponential, shape=1)", || {
        let gamma = Gamma::<f32>::exponential(1.0, false).unwrap();
        gamma.sample(Some(&[sample_size])).unwrap()
    }, iterations / 2); // Half iterations due to complexity
    
    // Gamma distribution (general case, shape=2)
    test_distribution_sampling("Gamma (shape=2)", || {
    ...
    let unit_values = Tensor::from_vec(
        (0..sample_size).map(|i| 0.01 + (i as f32) * 0.0098).collect(),
        vec![sample_size]
    );
    
    test_log_prob("Normal log_prob", || {
        let normal = Normal::<f32>::standard(false).unwrap();
        normal.log_prob(&values).unwrap()
    }, iterations);
    
    test_log_prob("Exponential log_prob", || {
        let exponential = Exponential::<f32>::standard(false).unwrap();
        exponential.log_prob(&positive_values).unwrap()
    }, iterations);
    
    test_log_prob("Gamma log_prob", || {
        let gamma = Gamma::<f32>::exponential(1.0, false).unwrap();
        gamma.log_prob(&positive_values).unwrap()
    }, iterations / 2);
    
    test_log_prob("Beta log_prob", || {
        let beta = Beta::<f32>::uniform(false).unwrap();
    ...
    test_stats("Normal統計計算", || {
        let normal = Normal::<f32>::standard(false).unwrap();
        (normal.mean().unwrap(), normal.variance().unwrap())
    }, iterations * 10);
    
    test_stats("Gamma統計計算", || {
        let gamma = Gamma::<f32>::exponential(1.0, false).unwrap();
    ...
    F: FnMut() -> Tensor<f32>,
{
    ...
    println!("{:<30} | {:>8.1} ns/op | {:>8.0} samples/s", 
