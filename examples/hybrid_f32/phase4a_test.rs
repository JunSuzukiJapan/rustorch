...
fn main() -> Result<(), Box<dyn std::error::Error>> {
    ...
    let sample_data = F32Tensor::from_vec(
        vec![1.2, 3.5, 2.1, 5.8, 4.2, 7.9, 6.1, 8.5, 9.2, 0.5, 3.8, 4.7, 6.3, 7.1, 2.9],
        vec![15]
    )?;
    ...
    println!("  Cumulative product: {:?}", cumprod.as_slice().iter().take(5).collect::<Vec<_>>());
    ...
    println!("  Moving std (window=3): {:?}", moving_std.as_slice().iter().map(|x| format!("{:.3}", x)).collect::<Vec<_>>());
    ...
    let normal_like = F32Tensor::from_vec(
        vec![2.1, 2.5, 3.0, 3.2, 3.5, 3.8, 4.0, 4.2, 4.5, 4.8, 5.0, 5.2, 5.5, 5.8, 6.0],
        vec![15]
    )?;
    ...
    println!("  Z-scores: {:?}", zscore.as_slice().iter().map(|x| format!("{:.3}", x)).collect::<Vec<_>>());
    ...
    let dataset_a = F32Tensor::from_vec(
        vec![3.0, 4.0, 5.0, 6.0, 7.0],
        vec![5]
    )?;
    let dataset_b = F32Tensor::from_vec(
        vec![2.8, 4.2, 5.1, 5.9, 7.2],
        vec![5]
    )?;
    ...
    let large_data: Vec<f32> = (0..1000).map(|i| (i as f32) * 0.01 + (i % 10) as f32).collect();
    ...
    println!("   ✓ Complete f32-specific advanced statistics implementation (zero conversion cost)");
