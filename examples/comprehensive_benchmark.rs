...
fn benchmark_tensor_operations() {
    ...
    for size in sizes {
        ...
        println!("    Addition (1000x): {:?} ({:.2} Mops/sec)", 
                add_time, 1000.0 / add_time.as_secs_f64() / 1_000_000.0);
        println!("    Sum (1000x): {:?} ({:.2} Mops/sec)", 
                sum_time, 1000.0 / sum_time.as_secs_f64() / 1_000_000.0);
        println!("    Mean (1000x): {:?} ({:.2} Mops/sec)", 
                mean_time, 1000.0 / mean_time.as_secs_f64() / 1_000_000.0);
    }
}
...
fn benchmark_matrix_operations() {
    ...
    for size in sizes {
        ...
        println!("    MatMul ({}x): {:?} ({:.2} GFLOPS)", 
                iterations, matmul_time, gflops);
        println!("    Transpose ({}x): {:?} ({:.2} ops/sec)", 
                iterations, transpose_time, iterations as f64 / transpose_time.as_secs_f64());
    }
}
...
fn benchmark_neural_networks() {
    ...
    for batch_size in batch_sizes {
        ...
        println!("    Forward pass ({}x): {:?} ({:.2} inferences/sec)", 
                iterations, forward_time, iterations as f64 / forward_time.as_secs_f64());
        println!("    Parameters: {}", params);
        println!("    Throughput: {:.2} samples/sec", 
...
fn benchmark_memory_operations() {
    ...
    for size in sizes {
        ...
        println!("  Size: {} elements", size);
        println!("    Creation (1000x): {:?} ({:.2} MB/sec)", 
                creation_time, 
                (size * 4 * 1000) as f64 / creation_time.as_secs_f64() / 1_000_000.0);
        println!("    Clone (1000x): {:?} ({:.2} MB/sec)", 
                clone_time,
                (size * 4 * 1000) as f64 / clone_time.as_secs_f64() / 1_000_000.0);
        println!("    Access (1000x): {:?} ({:.2} Mops/sec)", 
                access_time, 1000.0 / access_time.as_secs_f64() / 1_000_000.0);
    }
}
...
fn benchmark_advanced_operations() {
    ...
    // Reshape benchmark
    let matrix_tensor = Tensor::<f32>::from_vec(
    ...
    println!("    Broadcasting (1000x): {:?} ({:.2} Mops/sec)", 
            broadcast_time, 1000.0 / broadcast_time.as_secs_f64() / 1_000_000.0);
    println!("    Reshape (1000x): {:?} ({:.2} Mops/sec)", 
            reshape_time, 1000.0 / reshape_time.as_secs_f64() / 1_000_000.0);
    println!("    Statistics (1000x): {:?} ({:.2} Mops/sec)", 
            stats_time, 1000.0 / stats_time.as_secs_f64() / 1_000_000.0);
}