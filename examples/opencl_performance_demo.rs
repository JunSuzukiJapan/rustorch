...
use rustorch::gpu::opencl_optimized::{opencl_matmul_f32, OpenClMatrixExecutor};
...
fn display_device_info(executor: &OpenClMatrixExecutor) -> Result<(), Box<dyn std::error::Error>> {
    ...
    println!(
        "Local Memory: {:.1} KB",
        info.local_mem_size as f64 / 1024.0
    );
    ...
    match info.vendor.as_str() {
        ...
        v if v.contains("Intel") => {
            println!(
                "🔧 Intel GPU detected - Using conservative optimizations for integrated graphics"
            );
        }
        _ => {
...
fn test_matrix_multiplication_performance(
    executor: &mut OpenClMatrixExecutor,
    ...
    for &size in &sizes {
        ...
        if non_zero_count > expected_non_zero / 2 {
            println!(
                "  ✓ Correctness check passed ({} non-zero elements)",
                non_zero_count
            );
        } else {
            println!(
                "  ⚠️ Possible correctness issue ({} non-zero elements)",
                non_zero_count
            );
        }
    }
...
fn test_vendor_optimizations(
    ...
    for &(m, n, k, description) in &test_cases {
        ...
        let theoretical_gflops =
            info.compute_units as f64 * info.max_clock_frequency as f64 * 2.0 / 1000.0;
        let efficiency = (gflops / theoretical_gflops) * 100.0;

        println!(
            "  ✓ {:.3}ms ({:.1} GFLOPS, {:.1}% efficiency)",
            duration.as_secs_f64() * 1000.0,
            gflops,
            efficiency
        );
...
fn test_cross_platform_compatibility(
    ...
    println!(
        "Running compatibility tests on {} ({})",
    ...
    println!(
        "\nCompatibility Summary: {} passed, {} failed",
        passed, failed
    );
...
#[cfg(feature = "opencl")]
fn benchmark_opencl_performance(
    executor: &mut OpenClMatrixExecutor,
) -> Result<(), Box<dyn std::error::Error>> {
    ...
    for run in 1..=num_runs {
        ...
        println!(
            "Run {}: {:.3}ms ({:.0} GFLOPS, {:.1}% efficiency)",
            run,
            duration.as_secs_f64() * 1000.0,
            gflops,
            efficiency
    ...
    println!("\n📊 Benchmark Results:");
    println!(
        "Average: {:.3}ms ({:.0} GFLOPS, {:.1}% efficiency)",
        avg_duration * 1000.0,
        avg_gflops,
        avg_efficiency
    );
    ...
    if avg_efficiency > 50.0 {
        println!(
            "🟢 Excellent performance! OpenCL implementation is well-optimized for this device."
        );
    } else if avg_efficiency > 30.0 {
        println!(
            "🟡 Good performance. Further optimizations possible with device-specific tuning."
        );
    } else if avg_efficiency > 15.0 {
        println!(
            "🟠 Moderate performance. Consider different tile sizes or memory access patterns."
        );
    } else {
    ...
    match info.vendor.as_str() {
        ...
        _ => {
            println!(
                "💡 Generic optimization: Try different tile sizes and work group configurations."
            );
        }
    }
...
#[cfg(not(feature = "opencl"))]
fn test_matrix_multiplication_performance(_: &mut ()) -> Result<(), Box<dyn std::error::Error>> {
    Ok(())
}
#[cfg(not(feature = "opencl"))]
fn test_vendor_optimizations(_: &mut ()) -> Result<(), Box<dyn std::error::Error>> {
    Ok(())
}
#[cfg(not(feature = "opencl"))]
fn test_cross_platform_compatibility(_: &mut ()) -> Result<(), Box<dyn std::error::Error>> {
    Ok(())
}
#[cfg(not(feature = "opencl"))]
fn benchmark_opencl_performance(_: &mut ()) -> Result<(), Box<dyn std::error::Error>> {
    Ok(())
}
