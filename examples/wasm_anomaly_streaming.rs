...
fn main() {
    ...
    // Create detectors
    let statistical_detector = WasmAnomalyDetector::new(2.5, 100)
        .expect("Failed to create statistical detector");
    let timeseries_detector = WasmTimeSeriesDetector::new(50, Some(24))
        .expect("Failed to create time series detector");
    ...
    for i in 0..500 {
        ...
        if let Ok(Some(anomaly)) = statistical_detector.detect_realtime(value) {
            anomaly_count += 1;
            println!("🚨 Statistical anomaly at step {}: value={:.3}, anomaly={:?}", 
                     i, value, anomaly);
        } else {
            normal_count += 1;
        }
        ...
        if let Ok(Some(ts_anomaly)) = timeseries_detector.add_point(timestamp, value) {
            ...
            println!("📈 Time series anomaly at step {}: ts={}, value={:.3}, anomaly={:?}", 
    ...
    if let Ok(statistical_anomalies) = statistical_detector.detect_statistical(&batch_tensor) {
        println!("Statistical batch anomalies detected: {}", statistical_anomalies.len());
    ...
    if let Ok(isolation_anomalies) = statistical_detector.detect_isolation_forest(&batch_tensor, 100) {
        println!("Isolation Forest anomalies detected: {}", isolation_anomalies.len());
    ...
    println!("Original threshold: {:.2}", statistical_detector.get_threshold());
    ...
    if let Ok(sensitive_anomalies) = statistical_detector.detect_statistical(&batch_tensor) {
        println!("Anomalies with sensitive threshold: {}", sensitive_anomalies.len());
    }
