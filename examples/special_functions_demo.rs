...
fn main() -> Result<(), Box<dyn std::error::Error>> {
    ...
    println!(
        "Γ(0.5) = {} (should be √π ≈ 1.772)",
        gamma::gamma_scalar(0.5_f64)?
    );

    // Error function properties
    println!(
        "erf(1) = {} (should be ≈ 0.8427)",
        error::erf_scalar(1.0_f64)
    );
    ...
    println!(
        "J_0(0) = {} (should be 1)",
        bessel::bessel_j_scalar(0.0_f64, 0.0)?
    );
    println!(
        "I_0(0) = {} (should be 1)",
        bessel::bessel_i_scalar(0.0_f64, 0.0)?
    );
    ...
    println!(
        "erf({}) = {}, erf(-{}) = {}",
        x_val, erf_pos, x_val, erf_neg
    );
    println!(
        "Symmetry check: {} ≈ {} (should be equal)",
        erf_pos, -erf_neg
    );
    ...
    println!(
        "erfc({}) = {}, 1-erf({}) = {}",
        x_val, erfc_val, x_val, one_minus_erf
    );
    println!(
        "Identity check: difference = {}",
        (erfc_val - one_minus_erf).abs()
    );
