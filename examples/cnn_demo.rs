...
use rustorch::nn::{Sequential, Conv2d, MaxPool2d};
...
fn main() -> Result<(), Box<dyn std::error::Error>> {
    ...
    println!("\n📊 Input tensor shape: {:?}", input.data().read().unwrap().shape());
    ...
    println!("🔍 Conv2d output shape: {:?}", conv_output.data().read().unwrap().shape());
    
    // Test MaxPool2d
    let pool_test = MaxPool2d::new((2, 2), Some((2, 2)), Some((0, 0)));
    let pool_output = pool_test.forward(&conv_output);
    println!("🏊 MaxPool2d output shape: {:?}", pool_output.data().read().unwrap().shape());
