...
impl MNISTGenerator {
    ...
    fn generate_sample(&mut self) -> (Vec<f32>, usize) {
        ...
        for i in 0..784 {
            ...
            let col = i % 28;
            let center_distance =
                ((row as f32 - 14.0).powi(2) + (col as f32 - 14.0).powi(2)).sqrt();

            let pattern_value = match label {
                0 => {
                    if center_distance > 8.0 && center_distance < 12.0 {
                        0.8
                    } else {
                        0.1
                    }
                ...
                3 => {
                    if (row < 6) || (row > 10 && row < 16) || (row > 20) || (col > 15 && col < 25) {
                        0.8
                    } else {
                        0.1
                    }
                ...
                6 => {
                    if (row < 6) || (col < 8) || (row > 10 && row < 16 && col > 15) {
                        0.8
                    } else {
                        0.1
                    }
                ...
                8 => {
                    if (center_distance > 6.0 && center_distance < 10.0)
                        || (center_distance > 12.0 && center_distance < 16.0)
                    {
                        0.8
                    } else {
                        0.1
                ...
                9 => {
                    if (center_distance > 8.0 && center_distance < 12.0) || (col > 15 && row > 14) {
                        0.8
                    } else {
                        0.1
                    }
...
fn main() -> Result<(), Box<dyn std::error::Error>> {
    ...
    for epoch in 1..=epochs {
        ...
        println!(
            "✅ Epoch {} 完了 / completed: Loss: {:.4}  Accuracy: {:.1}%  Time: {:.2}s",
            epoch,
            avg_loss,
            avg_accuracy * 100.0,
    ...
    println!(
        "\n🎉 訓練完了 / Training completed! Total time: {:.2}s",
    ...
    println!(
        "📈 総サンプル数 / Total samples: {}",
        (train_batches + test_batches) * batch_size
    );
    ...
    println!(
        "🚀 処理速度 / Processing speed: {:.0} samples/sec",
        samples_per_second
    );
