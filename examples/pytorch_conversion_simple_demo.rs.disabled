...
fn create_sample_pytorch_model() -> PyTorchModel {
    ...
    model.set_architecture(
        "Simple MLP: Linear(784->128) -> BatchNorm -> ReLU -> Linear(128->10)".to_string(),
    );

    println!(
        "✅ Created PyTorch model with {} parameter tensors",
...
fn conversion_demo(pytorch_model: &PyTorchModel) -> Result<(), Box<dyn std::error::Error>> {
    ...
    println!("================");
    println!(
        "✅ Successfully converted {} layers",
        converted_model.layers.len()
    );
