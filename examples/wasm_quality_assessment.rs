...
fn main() {
    ...
    let quality_analyzer = WasmQualityMetrics::new(0.8).expect("Failed to create quality analyzer");
    let statistical_detector =
        WasmAnomalyDetector::new(2.0, 50).expect("Failed to create statistical detector");
    ...
    for (dataset_name, (data, shape, description)) in test_datasets {
        ...
        if let Ok(report) = quality_analyzer.quality_report(&tensor) {
            if let Ok(parsed_report) = serde_json::from_str::<serde_json::Value>(&report) {
                println!(
                    "   📋 Report: {}",
                    serde_json::to_string_pretty(&parsed_report).unwrap_or(report)
                );
            }
        }
        ...
        let variance =
            data_slice.iter().map(|&x| (x - mean).powi(2)).sum::<f32>() / data_slice.len() as f32;
        ...
        if sorted_data.len() >= 4 {
            ...
            let outliers: Vec<&f32> = data_slice
                .iter()
    ...
    for minute in 0..60 {
        ...
        if let Ok(quality_score) = quality_analyzer.overall_quality(&tensor) {
            if quality_score >= 0.7 {
                ...
                println!(
                    "   ⚠️  Minute {}: Quality {:.1}% (Poor - investigate)",
                    minute,
                    quality_score * 100.0
                );
        ...
        if let Ok(Some(anomaly)) = statistical_detector.detect_realtime(avg_value) {
            println!(
                "   🚨 Anomaly in minute {}: avg={:.3}, detail={:?}",
                minute, avg_value, anomaly
            );
        }
    }
    ...
    println!("\n--- Quality Monitoring Summary ---");
    println!(
        "Good quality minutes: {} ({:.1}%)",
        good_quality_count,
        (good_quality_count as f32 / 60.0) * 100.0
    );
    println!(
        "Poor quality minutes: {} ({:.1}%)",
        poor_quality_count,
    ...
    let strict_analyzer = WasmQualityMetrics::new(0.95).expect("Failed to create strict analyzer");
    let lenient_analyzer = WasmQualityMetrics::new(0.5).expect("Failed to create lenient analyzer");
...
fn create_high_quality_data() -> (Vec<f32>, Vec<usize>, &'static str) {
    ...
    (
        data,
        vec![1000],
        "Clean trending data with minor seasonal variation",
    )
}
...
fn create_medium_quality_data() -> (Vec<f32>, Vec<usize>, &'static str) {
    ...
    (
        data,
        vec![1000],
        "Trending data with moderate outliers and noise",
    )
}
...
fn create_low_quality_data() -> (Vec<f32>, Vec<usize>, &'static str) {
    ...
    (
        data,
        vec![1000],
        "Highly corrupted data with outliers and missing values",
    )
}
...
fn create_mixed_quality_data() -> (Vec<f32>, Vec<usize>, &'static str) {
    ...
    (
        data,
        vec![1000],
        "Mixed quality: clean first half, corrupted second half",
    )
}
...
fn create_realistic_data() -> (Vec<f32>, Vec<usize>, &'static str) {
    ...
    (
        data,
        vec![24, 60],
        "24-hour temperature sensor data with realistic issues",
    )
}
