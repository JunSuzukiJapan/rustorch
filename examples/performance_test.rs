...
fn benchmark_basic_ops() {
    ...
    for size in sizes {
        ...
        println!("    Addition (100x): {:?} ({:.2} ops/sec)", 
                add_time, 100.0 / add_time.as_secs_f64());
        println!("    Sum (100x): {:?} ({:.2} ops/sec)", 
                sum_time, 100.0 / sum_time.as_secs_f64());
    }
}
...
fn benchmark_nn_ops() {
    ...
    for batch_size in batch_sizes {
        ...
        println!("    Forward pass (50x): {:?} ({:.2} inferences/sec)", 
                forward_time, 50.0 / forward_time.as_secs_f64());
    }
}