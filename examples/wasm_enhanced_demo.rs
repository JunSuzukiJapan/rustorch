...
#[cfg(feature = "wasm")]
use rustorch::wasm::{special_enhanced::*, distributions_enhanced::*, optimizer_enhanced::*, autograd_simplified::*};
...
fn main() {
    ...
    for &x in &test_values {
        ...
        println!("  x={:.1}: Γ(x)={:.6}, ln(Γ(x))={:.6}, ψ(x)={:.6}", 
    ...
    for &x in &[0.5, 1.0, 2.0, 5.0] {
        ...
        println!("  x={:.1}: J_0={:.6}, Y_0={:.6}, I_0={:.6}, K_0={:.6}", 
                 x, j0, y0, i0, k0);
    }
    ...
    for &x in &[-2.0, -1.0, 0.0, 1.0, 2.0] {
        ...
        println!("  x={:.1}: erf(x)={:.6}, erfc(x)={:.6}", x, erf_val, erfc_val);
    }
    ...
    println!("Normal(0,1) mean: {:.6}, variance: {:.6}", normal.mean(), normal.variance());
    ...
    println!("Gamma(2,1.5) mean: {:.6}, variance: {:.6}", gamma_dist.mean(), gamma_dist.variance());
    ...
    println!("Beta(2,3) mean: {:.6}, variance: {:.6}", beta_dist.mean(), beta_dist.variance());
    ...
    println!("Statistics: mean={:.3}, variance={:.3}, skewness={:.3}, kurtosis={:.3}", 
    ...
    for step in [0, 10, 50, 100, 500, 1000] {
        ...
        println!("  Step {}: Exponential LR={:.6}, Cosine LR={:.6}", 
