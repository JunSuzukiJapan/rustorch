...
use rustorch::nn::{Embedding, PositionalEmbedding, SinusoidalPositionalEncoding, Module};
...
fn test_word_embedding() {
    ...
    println!("Created embedding: vocab_size={}, embedding_dim={}, padding_idx={:?}", 
             embedding.vocab_size(), embedding.embedding_dim(), embedding.padding_idx());
    
    // Create input with token indices [1, 5, 10, 0] (0 is padding)
    let input_indices = vec![1.0, 5.0, 10.0, 0.0];
    let input = Variable::new(
        Tensor::from_vec(input_indices, vec![4]),
        false
    );
    ...
    println!("Padding embedding norm: {:.6} (should be close to 0)", padding_embedding_norm);
...
fn test_positional_embedding() {
    ...
    println!("Created positional embedding: max_length={}, embedding_dim={}", 
             pos_embedding.max_length(), pos_embedding.embedding_dim());
...
fn test_sinusoidal_encoding() {
    ...
    println!("Created sinusoidal encoding: max_length={}, embedding_dim={}", 
             sin_encoding.max_length(), sin_encoding.embedding_dim());
    ...
    println!("Number of parameters: {} (should be 0 for fixed encoding)", params.len());
...
fn test_combined_embeddings() {
    ...
    println!("Word embedding: vocab_size={}, embedding_dim={}", 
             word_embedding.vocab_size(), word_embedding.embedding_dim());
    println!("Positional embedding: max_length={}, embedding_dim={}", 
             pos_embedding.max_length(), pos_embedding.embedding_dim());
    ...
    println!("Token input shape: {:?}", token_input.data().read().unwrap().shape());
    ...
    let word_embeddings = Variable::new(
        Tensor::from_vec(word_embeddings_data, vec![batch_size, seq_length, embedding_dim]),
        false
    );
    
    println!("Word embeddings shape: {:?}", word_embeddings.data().read().unwrap().shape());
    ...
    println!("Total parameters: {} (word: {}, positional: {})", 
             total_params, word_params.len(), pos_params.len());
}
...
mod tests {
    ...
    fn test_sinusoidal_encoding_basic() {
        ...
        let input = Variable::new(
            Tensor::from_vec(vec![0.1; 1*5*8], vec![1, 5, 8]),
            false
        );
