...
fn create_sample_pytorch_model() -> PyTorchModel {
    ...
    state_dict.tensors.insert("features.3.weight".to_string(), TensorData {
        shape: vec![64, 32, 3, 3],
        data: generate_random_data(64 * 32 * 3 * 3),
        dtype: "f32".to_string(),
    });
    state_dict.tensors.insert("features.3.bias".to_string(), TensorData {
        shape: vec![64],
    ...
    model.set_architecture("CNN for CIFAR-10: Conv2d -> BatchNorm -> ReLU -> Conv2d -> AdaptiveAvgPool -> Linear".to_string());
    
    println!("✅ Created PyTorch model with {} layers", model.layer_names().len());
