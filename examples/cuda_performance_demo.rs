...
use rustorch::gpu::cuda_enhanced::{CudaMatrixExecutor, cuda_matmul_f32, cuda_batch_matmul_f32};
...
fn display_device_info(executor: &CudaMatrixExecutor) -> Result<(), Box<dyn std::error::Error>> {
    ...
    match executor.get_compute_capability() {
        Ok((major, minor)) => {
            ...
            println!("Tensor Core Support: {}", if has_tensor_cores { "✓ Available" } else { "❌ Not Available" });
    ...
    match executor.get_memory_info() {
        Ok((free, total)) => {
            println!("GPU Memory: {:.1} GB total, {:.1} GB free", 
...
#[cfg(feature = "cuda")]
fn test_standard_matrix_multiplication(executor: &CudaMatrixExecutor) -> Result<(), Box<dyn std::error::Error>> {
    ...
    for &size in &sizes {
        ...
        if relative_error < 0.01 {
            println!("  ✓ Correctness check passed (error: {:.4}%)", relative_error * 100.0);
        } else {
...
#[cfg(feature = "cuda")]
fn test_tensor_core_acceleration(executor: &CudaMatrixExecutor) -> Result<(), Box<dyn std::error::Error>> {
    ...
    match executor.get_compute_capability() {
        Ok((major, minor)) if major > 7 || (major == 7 && minor >= 0) => {
            ...
            for &size in &sizes {
                ...
                // Create test matrices
                let a: Vec<f32> = (0..aligned_m*aligned_k).map(|i| (i as f32) / 1000.0).collect();
                let b: Vec<f32> = (0..aligned_k*aligned_n).map(|i| ((i * 3) as f32) / 1000.0).collect();
                let mut c_standard = vec![0.0f32; aligned_m * aligned_n];
                let mut c_tensor = vec![0.0f32; aligned_m * aligned_n];
                
                // Standard SGEMM
                let start = Instant::now();
                executor.matmul_f32(&a, &b, &mut c_standard, aligned_m, aligned_n, aligned_k, false)?;
                let standard_time = start.elapsed();
                
                // Tensor Core accelerated (simulated)
                let start = Instant::now();
                executor.matmul_f32(&a, &b, &mut c_tensor, aligned_m, aligned_n, aligned_k, true)?;
        ...
        Ok((major, minor)) => {
            println!("Tensor Cores not available on compute capability {}.{}", major, minor);
...
#[cfg(feature = "cuda")]
fn test_batch_matrix_multiplication(executor: &CudaMatrixExecutor) -> Result<(), Box<dyn std::error::Error>> {
    ...
    for &batch_size in &batch_sizes {
        ...
        for i in 0..batch_size {
            let a: Vec<f32> = (0..m*k).map(|j| ((i * 1000 + j) as f32) / 10000.0).collect();
            let b: Vec<f32> = (0..k*n).map(|j| (((i + 1) * 1000 + j * 2) as f32) / 10000.0).collect();
        ...
        println!("✓ Batch {} matrices ({}x{}x{}): {:.3}ms ({:.1} GFLOPS)", 
                 batch_size, m, n, k, duration.as_secs_f64() * 1000.0, gflops);
...
#[cfg(feature = "cuda")]
fn benchmark_cuda_performance(executor: &CudaMatrixExecutor) -> Result<(), Box<dyn std::error::Error>> {
    ...
    println!("Estimated peak performance: {:.0} GFLOPS", theoretical_gflops);
    ...
    for run in 1..=num_runs {
        ...
        println!("Run {}: {:.3}ms ({:.0} GFLOPS, {:.1}% of peak)", 
...
#[cfg(not(feature = "cuda"))]
fn test_standard_matrix_multiplication(_: &()) -> Result<(), Box<dyn std::error::Error>> { Ok(()) }
#[cfg(not(feature = "cuda"))]  
fn test_tensor_core_acceleration(_: &()) -> Result<(), Box<dyn std::error::Error>> { Ok(()) }
#[cfg(not(feature = "cuda"))]
fn test_batch_matrix_multiplication(_: &()) -> Result<(), Box<dyn std::error::Error>> { Ok(()) }
#[cfg(not(feature = "cuda"))]
fn benchmark_cuda_performance(_: &()) -> Result<(), Box<dyn std::error::Error>> { Ok(()) }