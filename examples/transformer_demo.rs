...
use rustorch::nn::{
    Module, Embedding, SinusoidalPositionalEncoding, LayerNorm, 
...
fn test_layer_norm() {
    ...
    println!("Created LayerNorm: normalized_shape={:?}, eps={:.2e}, elementwise_affine={}", 
             layer_norm.normalized_shape(), layer_norm.eps(), layer_norm.elementwise_affine());
    ...
    println!("Number of parameters: {} (should be 2: weight + bias)", params.len());
}
...
fn test_multi_head_attention() {
    ...
    println!("Created MultiHeadAttention: d_model={}, num_heads={}, d_k={}", 
             mha.d_model(), mha.num_heads(), mha.d_k());
    ...
    println!("Number of parameters: {} (should be 8: 4 layers × 2 params each)", params.len());
}
...
fn test_transformer_encoder_layer() {
    ...
    println!("Created TransformerEncoderLayer: d_model={}, d_ff={}", 
             encoder_layer.d_model(), encoder_layer.d_ff());
    ...
    println!("Number of parameters: {} (multiple components)", params.len());
}
...
fn test_complete_transformer_pipeline() {
    ...
    println!("  vocab_size: {}, d_model: {}, num_heads: {}", vocab_size, d_model, num_heads);
    println!("  d_ff: {}, num_layers: {}, max_length: {}", d_ff, num_layers, max_length);
    ...
    let transformer_encoder = TransformerEncoder::<f32>::new(num_layers, d_model, num_heads, d_ff, None);
    ...
    println!("\nStep 1: Token input shape: {:?}", token_input.data().read().unwrap().shape());
    ...
    for b in 0..batch_size {
        for s in 0..seq_length {
            ...
            let single_token = Variable::new(
                Tensor::from_vec(vec![token_idx], vec![1]),
                false
            );
    ...
    println!("Step 2: Word embeddings shape: {:?}", word_embeddings.data().read().unwrap().shape());
    
    // Step 5: Add positional encoding
    let positioned_embeddings = pos_encoding.forward(&word_embeddings);
    
    println!("Step 3: Positioned embeddings shape: {:?}", positioned_embeddings.data().read().unwrap().shape());
    
    // Step 6: Pass through Transformer encoder
    let encoder_output = transformer_encoder.forward(&positioned_embeddings, None);
    
    println!("Step 4: Transformer output shape: {:?}", encoder_output.data().read().unwrap().shape());
    ...
    println!("  Transformer encoder: {} parameters", transformer_params.len());
