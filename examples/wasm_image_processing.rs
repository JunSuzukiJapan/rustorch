...
fn main() {
                ...
                if let Ok(cropped) = crop_transform.apply(&resized) {
                    ...
                    if let Ok(jitter_transform) = WasmColorJitter::new(0.2, 0.2, 0.1, 0.05) {
                        if let Ok(jittered) = jitter_transform.apply(&cropped) {
                            ...
                            if let Ok(normalize_transform) =
                                WasmNormalize::new(&imagenet_mean, &imagenet_std)
                            {
                                if let Ok(normalized) = normalize_transform.apply(&jittered) {
                                    println!(
                                        "   Normalized range: {:.3} - {:.3}",
                                        normalized
                                            .data()
                                            .iter()
                                            .fold(f32::INFINITY, |a, &b| a.min(b)),
                                    ...
                                    if let Ok(tensor_transform) = WasmToTensor::new() {
                                        if let Ok(final_tensor) =
                                            tensor_transform.apply(&normalized)
                                        {
                                            println!(
                                                "   Final tensor shape: {:?}",
                                                final_tensor.shape()
    ...
    if let Ok(pipeline) = WasmTransformPipeline::new(true) {
        // Enable caching
