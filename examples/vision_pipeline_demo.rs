...
fn create_sample_image(height: usize, width: usize) -> Result<Image<f32>, Box<dyn std::error::Error>> {
    ...
    for c in 0..3 { // RGB channels
        for h in 0..height {
            for w in 0..width {
                let value = (h as f32 / height as f32 + w as f32 / width as f32 + c as f32 * 0.1) / 2.0;
...
fn demo_conditional_transforms(
    ...
    // Test on small image
    // 小さい画像でテスト
    println!("    Small image ({}x{})", small_image.width, small_image.height);
    let small_result = pipeline.apply(small_image)?;
    println!("      Output: {:?}", small_result.data.shape());
    
    // Test on large image
    // 大きい画像でテスト
    println!("    Large image ({}x{})", large_image.width, large_image.height);
...
fn demo_preset_pipelines(image: &Image<f32>) -> Result<(), Box<dyn std::error::Error>> {
    ...
    println!("    ImageNet Training: {} transforms → {:?}", 
             imagenet_pipeline.len(), result.data.shape());
    
    // CIFAR training pipeline
    // CIFAR訓練パイプライン
    let cifar_pipeline = CIFARPreprocessing::training();
    let result = cifar_pipeline.apply(image)?;
    println!("    CIFAR Training: {} transforms → {:?}", 
             cifar_pipeline.len(), result.data.shape());
    ...
    println!("    Mobile Optimized: {} transforms → {:?} (cache: {}/{})", 
             mobile_pipeline.len(), result.data.shape(), cache_size, max_cache);
...
fn demo_performance_monitoring(image: &Image<f32>) -> Result<(), Box<dyn std::error::Error>> {
    ...
    for i in 1..=5 {
        ...
        println!("    Iteration {}: {:?} | Cache: {}/{} | Hits: {} | Misses: {}", 
    ...
    println!("    Average time: {:.2}μs", final_stats.avg_processing_time_us);
...
fn demo_batch_processing(images: Vec<Image<f32>>) -> Result<(), Box<dyn std::error::Error>> {
    ...
    println!("    Average per image: {:?}", batch_time / images.len() as u32);
