...
fn main() {
    ...
    println!("w: {:?}, requires_grad: {}", w.data().read().unwrap().as_array(), w.requires_grad());
    println!("x: {:?}, requires_grad: {}", x.data().read().unwrap().as_array(), x.requires_grad());
    
    // Test matmul: y = x @ w
    let y = x.matmul(&w);
    println!("y = x @ w: {:?}, requires_grad: {}", y.data().read().unwrap().as_array(), y.requires_grad());
    
    // Backward
    println!("\n=== Testing Matmul Backward ===");
    println!("Before backward - w grad: {:?}", w.grad().read().unwrap().as_ref().map(|g| g.as_array()));
    
    y.backward();
    
    println!("After backward - w grad: {:?}", w.grad().read().unwrap().as_ref().map(|g| g.as_array()));
    ...
    println!("Before backward - w2 grad: {:?}", w2.grad().read().unwrap().as_ref().map(|g| g.as_array()));
    
    loss.backward();
    
    println!("After backward - w2 grad: {:?}", w2.grad().read().unwrap().as_ref().map(|g| g.as_array()));
