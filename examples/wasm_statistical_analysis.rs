...
fn main() {
    ...
    let datasets = vec![
        ("normal_distribution", generate_normal_distribution(1000, 50.0, 15.0)),
    ...
    for (dataset_name, (data, shape, description)) in datasets {
        ...
        println!("Shape: {:?}, Elements: {}", tensor.shape(), tensor.data().len());
        ...
        if let Ok(percentile_values) = statistical_analyzer.percentiles(&tensor, percentiles.clone()) {
            println!("📊 Percentiles:");
            ...
            println!("   Outlier bounds: [{:.3}, {:.3}]", lower_bound, upper_bound);
        }
        ...
        if let Some(ref log_data) = log_tensor {
            ...
            let log_max = log_data.data().iter().fold(f32::NEG_INFINITY, |a, &b| a.max(b));
            println!("   Log transform range: [{:.3}, {:.3}]", log_range, log_max);
        }
        
        // Skewness approximation using percentiles
        if let Ok(percentile_values) = statistical_analyzer.percentiles(&tensor, vec![25.0, 50.0, 75.0]) {
        ...
        if shape.len() == 2 && shape[1] >= 2 {
            ...
            let col1_data: Vec<f32> = tensor.data().iter()
                .enumerate()
                .filter(|(i, _)| i % shape[1] == 0)
                .map(|(_, &v)| v)
                .collect();
            let col2_data: Vec<f32> = tensor.data().iter()
                .enumerate()
            ...
            if col1_data.len() == col2_data.len() && col1_data.len() > 1 {
                ...
                if let Ok(correlation) = statistical_functions.correlation(&col1_tensor, &col2_tensor) {
        ...
        if let Ok(percentiles) = statistical_analyzer.percentiles(&tensor, vec![2.5, 25.0, 50.0, 75.0, 97.5]) {
            ...
            println!("   Q1-Median ratio: {:.3} (expect ~1.0 for normal)", expected_q1_median);
            println!("   Tail symmetry: {:.3} (expect ~1.0 for normal)", tail_symmetry);
            
            let normality_score = 1.0 - ((expected_q1_median - 1.0).abs() + (tail_symmetry - 1.0).abs()) / 2.0;
    ...
    if normal_tensor.data().len() == uniform_tensor.data().len() {
        if let Ok(cross_correlation) = statistical_functions.correlation(&normal_tensor, &uniform_tensor) {
    ...
    if let Ok(stats_json) = serde_json::from_str::<serde_json::Value>(&final_stats) {
        ...
        println!("   Memory saved: {:.2} MB", memory_saved as f32 / (1024.0 * 1024.0));
...
fn generate_normal_distribution(n: usize, mean: f32, std: f32) -> (Vec<f32>, Vec<usize>, &'static str) {
    ...
    (data, vec![n], "Normal distribution using Box-Muller transform")
}
...
fn generate_bimodal_distribution(n: usize) -> (Vec<f32>, Vec<usize>, &'static str) {
    ...
    (data, vec![n], "Bimodal distribution with modes at -2 and +3")
}
...
fn generate_time_series(n: usize, with_seasonality: bool) -> (Vec<f32>, Vec<usize>, &'static str) {
    ...
    (data, vec![n], "Time series with trend, seasonality, and noise")
}
...
fn generate_financial_returns(n: usize) -> (Vec<f32>, Vec<usize>, &'static str) {
    ...
    (data, vec![n], "Simulated daily stock returns with volatility clustering")
}
...
fn generate_experimental_data(n: usize) -> (Vec<f32>, Vec<usize>, &'static str) {
    ...
    for measurement in 0..n {
        ...
        let measured_value = true_value + systematic_error + random_error + measurement_error + environmental;
        data.push(measured_value);
    }
    
    (data, vec![n], "Experimental measurements with systematic and random errors")
}

/// Generate uniform distribution
fn generate_uniform_distribution(n: usize, min: f32, max: f32) -> (Vec<f32>, Vec<usize>, &'static str) {
    let mut data = Vec::with_capacity(n);
    let range = max - min;
    ...
    (data, vec![n], "Uniform distribution using linear congruential generator")
}
