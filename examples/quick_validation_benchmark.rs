...
fn main() -> rustorch::error::RusTorchResult<()> {
    ...
    println!("🔥 Quick Test - {}x{} matrix, {} iterations", test_size, test_size, iterations);
    println!("=======================================");
    ...
    // 軽量演算チェーン
    let perform_light_operations_f32 = |a: &F32Tensor, b: &F32Tensor, executor: &mut F32HybridExecutor| -> rustorch::error::RusTorchResult<f64> {
    ...
    let perform_light_operations_f64 = |a: &Tensor<f64>, b: &Tensor<f64>| -> rustorch::error::RusTorchResult<f64> {
        let start = Instant::now();
    ...
    // 既存ハイブリッド演算（CPU禁止）
    let perform_true_hybrid_operations = |a: &Tensor<f64>, b: &Tensor<f64>, _executor: &HybridExecutor| -> rustorch::error::RusTorchResult<f64> {
    ...
    // 2️⃣ Metal GPU単体実行
    println!("\n⚡ Metal GPU-Only Test:");
    let metal_time = perform_light_operations_f32(&matrix_a_f32, &matrix_b_f32, &mut hybrid_executor)?;
    println!("  ⚡ Metal GPU operations: {:.0}ms", metal_time);

    // 3️⃣ 真の既存ハイブリッド実行
    println!("\n🔄 True Existing Hybrid Test:");
    let existing_time = perform_true_hybrid_operations(&matrix_a_f64, &matrix_b_f64, &existing_hybrid_executor)?;
    println!("  🔄 True existing hybrid operations: {:.0}ms", existing_time);

    // 4️⃣ hybrid_f32実行
    println!("\n🚀 Hybrid_f32 Test:");
    let f32_time = perform_light_operations_f32(&matrix_a_f32, &matrix_b_f32, &mut hybrid_executor)?;
    println!("  🚀 Hybrid_f32 operations: {:.0}ms", f32_time);

    // 📊 結果分析
    println!("\n📊 Quick Validation Results for {}x{} matrix:", test_size, test_size);
    ...
    println!("\n🔬 Hybrid Comparison:");
    println!("  True Existing Hybrid vs Hybrid_f32: {:.2}x ratio", existing_vs_f32);
